...
begin
  ...
  try
    ...
    begin
      ...
      dmMSProcedure.fdspMaster.ParamByName('I_SQL').Value := 'UPDATE PRODUTOFILIAL SET  ALTERACAO = ' + QuotedStr(FormatDateTime('mm/dd/yy',cdsCloneCLD_NFE.FieldByName('ALTERACAO').Value)) + ', '
                                                             ...
                                                             + 'REMARCACAO = ' + QuotedStr(FormatDateTime('mm/dd/yy',cdsCloneCLD_NFE.FieldByName('REMARCACAO').Value)) + ', '
                                                             + 'ULTIMA_ENTRADA = ' + QuotedStr(FormatDateTime('mm/dd/yy',cdsCloneCLD_NFE.FieldByName('ULTIMA_ENTRADA').Value)) + ', '
...
{      if dmMProvider.cdsConfiguracoesATUALIZAR_FOR_PRO_PELA_NFE.Value = 1 then
    ...
    lblMsg.Caption := 'Atualização completa!';
    application.ProcessMessages;
...
procedure TfrmCalculoCLD_NFE.Mensagem(pAlerta: boolean; pMsg: string);
begin
...
procedure TfrmCalculoCLD_NFE.Tratar_Erro_Conexao(pE: exception);
var
...
begin
  ...
  begin
    ...
    sChave_Extrang :=
      Trim(Copy(pE.Message, Pos('ON TABLE', UpperCase(pE.Message)) + 10,
      Length(pE.Message)));

    Mensagem(false, dmDBEXMaster.sNomeUsuario + ', ' +
      MSG_ERRO_CHAVE_ESTRANGEIRA + Copy(sChave_Extrang, 1,
      Pos('"', sChave_Extrang) - 1));
