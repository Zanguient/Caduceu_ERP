...
type
  ...
  TfrmManutencaoRegSintegra = class(TForm)
    ...
    procedure grdReg50KeyPress(Sender: TObject; var Key: Char);
    procedure grdReg54KeyPress(Sender: TObject; var Key: Char);
  private
...
uses dataDBEXMaster, dataMProvider, dataMSource, dataMSProcedure,
  uConstantes_Padrao, uFuncoes, modNFEntrada;
...
procedure TfrmManutencaoRegSintegra.grdReg50KeyPress(Sender: TObject;
  var Key: Char);
begin
  ...
  if key = FormatSettings.DecimalSeparator then
    ...
    grdReg50.Perform(WM_KEYDOWN, VK_TAB, 0);

end;

procedure TfrmManutencaoRegSintegra.grdReg54KeyPress(Sender: TObject;
  var Key: Char);
begin
...
begin
  ...
  if frmModNFEntrada.GravarEstornarLivro_Entrada(1) then
    frmModNFEntrada.GravarEstornarLivro_Entrada(0);
