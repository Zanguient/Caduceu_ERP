...
uses
  ...
  Vcl.Grids, Vcl.DBGrids, Vcl.Mask, Vcl.DBCtrls, Data.DB, Vcl.Buttons, Vcl.Imaging.jpeg, CPort;
...
type
  ...
  TfrmProdutos = class(TForm)
    ...
    procedure tbsInfoAdicionaisShow(Sender: TObject);
    ...
    procedure cboOrigemMercadoriaExit(Sender: TObject);
  ...
  private
    ...
    PROCEDURE AbrirTabelaPIS_COFIN;
    ...
    procedure RelacionarCST_CSOSN;
    procedure RelacionarFornecedor;
...
uses dataDBEXMaster, dataMProvider, dataMSource, dataMSProcedure, modPesquisaFornecedorCNPJRSocial, uConstantes_Padrao, uFuncoes, modPesquisaBarras, modMVA,
      ...
      modPesquisaTributacao, modIngredientes, modTeclado_Balanca, modMostrarSimilares, modSelecionarSimilar, uFuncoes_BD, modProduto_DJPDV;

procedure TfrmProdutos.AbrirTabelaPIS_COFIN;
begin

  dmMProvider.cdsCST_PIS_ENT.Close;
  dmMProvider.cdsCST_PIS_ENT.ProviderName := 'dspCST_PIS_ENT';
  dmMProvider.cdsCST_PIS_ENT.Open;
  dmMProvider.cdsCST_PIS_ENT.ProviderName := '';

  dmMProvider.cdsCST_PIS_SAI.Close;
  dmMProvider.cdsCST_PIS_SAI.ProviderName := 'dspCST_PIS_SAI';
  dmMProvider.cdsCST_PIS_SAI.Open;
  dmMProvider.cdsCST_PIS_SAI.ProviderName := '';

  dmMProvider.cdsCST_COFINS_ENT.Close;
  dmMProvider.cdsCST_COFINS_ENT.ProviderName := 'dspCST_COFINS_ENT';
  dmMProvider.cdsCST_COFINS_ENT.Open;
  dmMProvider.cdsCST_COFINS_ENT.ProviderName := '';

  dmMProvider.cdsCST_COFINS_SAI.Close;
  dmMProvider.cdsCST_COFINS_SAI.ProviderName := 'dspCST_COFINS_SAI';
  dmMProvider.cdsCST_COFINS_SAI.Open;
  dmMProvider.cdsCST_COFINS_SAI.ProviderName := '';
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          Man_Tab_Barras(1,dmMProvider.cdsBarrasBARRAS.Value, dmMProvider.cdsBarrasBARRAS.Value,dmMProvider.cdsProdutosPRODUTO.AsInteger);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if trim(dmDBEXMaster.fdqBarras.FieldByName('BARRAS').AsString) <> '' then
        begin
...
begin
  ...
  begin
    ...
    begin
      ...
      if not dmMProvider.cdsProdutos.Active then
        AbrirTabelaProdutos(-1);

      if not dmMProvider.cdsProdutoFilial.Active then
        AbrirTabelaFilial(-1);
      ...
      if Length(Trim(frmPesquisaBarras.edtCodigoBarras.Text)) > 0 then
        ...
        dmDBEXMaster.bCodigoGerado := True;
...
begin
  ...
  begin
    ...
    begin
      ...
      if Length(Trim(cboOrigemMercadoria.Text)) <= 0 then
      begin
    ...
    if InserindoEditando(dmMProvider.cdsProdutoFilial) then
      dmMProvider.cdsProdutoFilial.Post;
  ...
  begin
    ...
    if dmMProvider.cdsProdutoFilialSETORDEBALANCA.Value = 1 then
      dmMProvider.cdsBarrasPESADO.Value     := 1
    else
      dmMProvider.cdsBarrasPESADO.Value     := 0;
  ...
  iCod_Produto := dmMProvider.cdsProdutosPRODUTO.Value;
  ...
  dmDBEXMaster.fdqProdutos.SQL.Add('WHERE PRO.PRODUTO = ' + IntToStr(iCod_Produto));
...
procedure TfrmProdutos.cboOrigemMercadoriaExit(Sender: TObject);
begin
...
begin
  ...
  begin
    ...
    else if Length(Trim(dmMProvider.cdsProdutosUNIDADE.Value)) <= 0 then
    begin

      lblMsg.Caption := dmDBEXMaster.sNomeUsuario + MSG_CAMPO_OBRIGATORIO;
      Application.ProcessMessages;
      cboUnidadeMedida.SetFocus;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        dmMProvider.cdsProdutosCST.Value            := FormatFloat('000', StrToFloat(dmMProvider.cdsProdutosCST.Value));
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          Application.MessageBox(pchar('Descrição NCM' + #13 + #13 + dmMProvider.cdsNcmNbsNOME_NCM.AsString + #13 +#13
                                  + 'Teve sua vigência encerrada em: ' + FormatDateTime('dd/mm/yyyy',dmMProvider.cdsNcmNbsDATA_TERMINO.Value) + #13 +#13
                                  + 'Informe um NCM vigente'),'Informação!', mb_IconInformation + mb_ok);
          edtNCM_TIPI.SetFocus;
        end
        else
        begin

          Application.MessageBox(pchar('Descrição NCM' + #13 + #13 + dmMProvider.cdsNcmNbsNOME_NCM.AsString),'Informação!', mb_IconInformation + mb_ok);
...
begin
  ...
  else if (UpperCase(Key) = 'C') and (dmMProvider.cdsProdutosGRUPO.Value > 0) then
  begin
  ...
  begin
    ...
    lblMsg.Caption := dmDBEXMaster.sNomeUsuario + MSG_INFORMAR_GRUPO_PRODUTO;
    Application.ProcessMessages;
    edtSubGrupo.SetFocus;
...
begin
  ...
  begin
    ...
    begin
      ...
      else if Trim(dmMProvider.cdsTributacaoCODIGOECF.Value) = '99' then
      begin
        ...
        begin
          ...
          dmMProvider.cdsProdutosNOME_TRIBUTACAO.Value      := dmMProvider.cdsTributacaoNOME.Value;

          Application.MessageBox(pchar(dmDBEXMaster.sNomeUsuario + ', esse item não será enviado ao Aplicativo PAF-ECF. A alíquota informada: '
                                        + FormatFloat('#0.00',dmMProvider.cdsTributacaoALIQUOTA.Value)+'%'
                                        + ' não está cadastrada no ECF!'),'Informação'  , mb_IconInformation + mb_ok);
...
begin
  ...
  try
    ...
    dmMSProcedure.fdspBarras.Params[3].Value  := dmMProvider.cdsBarrasPESADO.Value;
    dmMSProcedure.fdspBarras.Params[4].Value  := dmMProvider.cdsBarrasGERADO.Value;
    ...
    begin
      ...
      lblMsg.Caption  := dmDBEXMaster.sNomeUsuario + '-' + pBarras + ' - ' + Tratar_Erro_Conexao(E);
      Result          := False;
      Application.ProcessMessages;
...
begin
  ...
  try
    ...
    dmMSProcedure.fdspProdutoFilial.Params[1].Value       := dmMProvider.cdsProdutoFilialPRODUTO.Value; //dmMProvider.cdsProdutosPRODUTO.AsInteger;
...
begin
  ...
  try
    ...
    dmMSProcedure.fdspProdutos.Params[37].Value     := dmMProvider.cdsProdutosUND_ATACADO.Value;
    ...
    begin
      ...
      dmMProvider.cdsProdutoFilialPRODUTO.Value := dmMProvider.cdsProdutosPRODUTO.Value;

    end;

    if  Length(Trim(sBarras)) <= 0 then
      sBarras := sCodigoBarrasGerado;
    ...
    case dmMProvider.cdsConfiguracoesMODELOPDV.Value of
      4:begin
          ...
          ExportarProdutosDJPDV(dmMProvider.cdsParamIntegraPDVARQUIVO_PRODUTOS.Value, dmMProvider.cdsParamIntegraPDVPASTA_EXPORTACAO.Value, dmDBEXMaster.iIdFilial, dmMProvider.cdsParamIntegraPDVTIPO_CODIGO_INTEGRACAO.Value);
...
begin
  ...
  if dmMProvider.cdsGrupoCUSTOOPERACIONAL.Value > 0 then
...
begin
  ...
  begin
    ...
    if (Length(dmMProvider.cdsBarrasBARRAS.Value) > 8) and (Length(dmMProvider.cdsBarrasBARRAS.Value) < 14) then
    begin
...
procedure TfrmProdutos.RelacionarCST_CSOSN;
begin
...
procedure TfrmProdutos.RelacionarFornecedor;
begin
...
procedure TfrmProdutos.tbsCadastroShow(Sender: TObject);
begin

  if (not dmMProvider.cdsProdutos.IsEmpty) and (not InserindoEditando(dmMProvider.cdsProdutos)) and (not InserindoEditando(dmMProvider.cdsProdutoFilial)) then
  begin
...
procedure TfrmProdutos.tbsInfoAdicionaisShow(Sender: TObject);
begin
...
begin
  ...
  begin
    ...
    if dmMProvider.cdsProdutosCST_PIS_ENTRADA.Value <= 0 then
    begin
  ...
  else if Length(Trim(dmMProvider.cdsProdutosCST.Value))  < 0 then
  begin
  ...
  else if Length(Trim(dmMProvider.cdsProdutosNCM.Value)) <= 0 then
  begin
