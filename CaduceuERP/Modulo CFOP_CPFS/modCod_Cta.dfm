object frmPlanoContaContabeis: TfrmPlanoContaContabeis
  Left = 0
  Top = 0
  ActiveControl = edtNumeroContaContabil
  BorderIcons = [biSystemMenu, biMinimize]
  BorderStyle = bsSingle
  Caption = 'Plano de Ciontas Cont'#225'beis'
  ClientHeight = 371
  ClientWidth = 730
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  object imgSair: TImage
    Left = 673
    Top = 320
    Width = 51
    Height = 44
    Cursor = crHandPoint
    Hint = 'Sair'
    ParentShowHint = False
    Picture.Data = {
      }
    ShowHint = True
    Stretch = True
    OnClick = imgSairClick
  end
  object imgConfirmar: TImage
    Left = 673
    Top = 8
    Width = 51
    Height = 44
    Cursor = crHandPoint
    Hint = 'Confirmar'
    ParentShowHint = False
    Picture.Data = {
      
      0FD78E0000000049454E44AE426082}
    ShowHint = True
    Stretch = True
    OnClick = imgConfirmarClick
  end
  object grpMensagem: TGroupBox
    Left = 8
    Top = 327
    Width = 662
    Height = 37
    Caption = 'Mensagem'
    TabOrder = 0
    object lblMsg: TLabel
      Left = 80
      Top = 14
      Width = 535
      Height = 16
      ParentCustomHint = False
      AutoSize = False
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -12
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
    end
  end
  object PageControl1: TPageControl
    Left = 8
    Top = 8
    Width = 662
    Height = 313
    ActivePage = TabSheet1
    TabOrder = 1
    object TabSheet1: TTabSheet
      object lblNomeContaContabil: TLabel
        Left = 8
        Top = 64
        Width = 57
        Height = 13
        Caption = 'Nome conta'
      end
      object lblDataInclusaoAlteracao: TLabel
        Left = 8
        Top = 16
        Width = 113
        Height = 13
        Caption = 'Data inclus'#227'o/altera'#231#227'o'
        FocusControl = edtDataInclusaoAlteracao
      end
      object lblNumeroContaContabil: TLabel
        Left = 8
        Top = 40
        Width = 107
        Height = 13
        Caption = 'N'#250'mero conta cont'#225'bil'
        FocusControl = edtNumeroContaContabil
      end
      object lblCNPJEStabelecimento: TLabel
        Left = 8
        Top = 265
        Width = 121
        Height = 13
        Caption = 'CNPJ do Estabelecimento'
        FocusControl = edtCNPJEstabelecimento
      end
      object lblContaRelaPlanoContasReferRFB: TLabel
        Left = 8
        Top = 241
        Width = 224
        Height = 13
        Caption = 'Conta relac. Plano de contas referenciada RFB'
        FocusControl = edtContareferenciadaRFB
      end
      object lblNivelConta: TLabel
        Left = 8
        Top = 217
        Width = 68
        Height = 13
        Caption = 'N'#237'vel da conta'
        FocusControl = edtNivelConta
      end
      object edtDataInclusaoAlteracao: TDBEdit
        Left = 249
        Top = 13
        Width = 80
        Height = 21
        Color = 14079702
        DataField = 'DATA_CADASTRO'
        DataSource = dmMSource.dtsSped_R0500
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        ParentFont = False
        ReadOnly = True
        TabOrder = 0
        OnKeyPress = edtDataInclusaoAlteracaoKeyPress
      end
      object edtNumeroContaContabil: TDBEdit
        Left = 249
        Top = 37
        Width = 381
        Height = 21
        DataField = 'COD_CTA'
        DataSource = dmMSource.dtsSped_R0500
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        TabOrder = 1
        OnEnter = edtNumeroContaContabilEnter
        OnExit = edtNumeroContaContabilExit
        OnKeyDown = edtNumeroContaContabilKeyDown
        OnKeyPress = edtDataInclusaoAlteracaoKeyPress
      end
      object edtNomeConta: TDBEdit
        Left = 249
        Top = 61
        Width = 381
        Height = 21
        CharCase = ecUpperCase
        DataField = 'NOME_CTA'
        DataSource = dmMSource.dtsSped_R0500
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        TabOrder = 2
        OnEnter = edtNumeroContaContabilEnter
        OnExit = edtNumeroContaContabilExit
        OnKeyDown = edtNumeroContaContabilKeyDown
        OnKeyPress = edtDataInclusaoAlteracaoKeyPress
      end
      object rdgTipoConta: TDBRadioGroup
        Left = 249
        Top = 88
        Width = 381
        Height = 40
        Caption = 'Tipo da conta'
        Columns = 2
        DataField = 'IND_CTA'
        DataSource = dmMSource.dtsSped_R0500
        Items.Strings = (
          'Anal'#237'tica (conta)'
          'Sint'#233'tica (grupo de contas)')
        TabOrder = 3
        Values.Strings = (
          'A'
          'S')
      end
      object rdgNaturezadaConta: TDBRadioGroup
        Left = 249
        Top = 129
        Width = 381
        Height = 79
        Caption = 'Natureza da conta'
        Columns = 2
        DataField = 'COD_NAT_CC'
        DataSource = dmMSource.dtsSped_R0500
        Items.Strings = (
          'Contas de ativo'
          'Contas de passivo'
          'Patrim'#244'nio l'#237'quido'
          'Contas de resultado'
          'Contas de compensa'#231#227'o'
          'Outras')
        TabOrder = 4
        Values.Strings = (
          '1'
          '2'
          '3'
          '4'
          '5'
          '9')
      end
      object edtNivelConta: TDBEdit
        Left = 249
        Top = 214
        Width = 381
        Height = 21
        DataField = 'NIVEL'
        DataSource = dmMSource.dtsSped_R0500
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        MaxLength = 5
        ParentFont = False
        TabOrder = 5
        OnEnter = edtNumeroContaContabilEnter
        OnExit = edtNumeroContaContabilExit
        OnKeyDown = edtNumeroContaContabilKeyDown
        OnKeyPress = edtNivelContaKeyPress
      end
      object edtContareferenciadaRFB: TDBEdit
        Left = 249
        Top = 238
        Width = 381
        Height = 21
        DataField = 'COD_CTA_REFER'
        DataSource = dmMSource.dtsSped_R0500
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        MaxLength = 60
        ParentFont = False
        TabOrder = 6
        OnEnter = edtNumeroContaContabilEnter
        OnExit = edtNumeroContaContabilExit
        OnKeyDown = edtNumeroContaContabilKeyDown
        OnKeyPress = edtDataInclusaoAlteracaoKeyPress
      end
      object edtCNPJEstabelecimento: TDBEdit
        Left = 249
        Top = 262
        Width = 192
        Height = 21
        DataField = 'CNPJ_EST'
        DataSource = dmMSource.dtsSped_R0500
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        MaxLength = 14
        ParentFont = False
        TabOrder = 7
        OnEnter = edtNumeroContaContabilEnter
        OnExit = edtNumeroContaContabilExit
        OnKeyDown = edtNumeroContaContabilKeyDown
        OnKeyPress = edtNivelContaKeyPress
      end
    end
  end
  object ACBrValidador1: TACBrValidador
    IgnorarChar = './-'
    Left = 72
    Top = 152
  end
end
