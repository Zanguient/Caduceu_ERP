object frmCFOP_CFPS: TfrmCFOP_CFPS
  Left = 0
  Top = 0
  ActiveControl = edtConteudoPesquisa
  BorderIcons = [biSystemMenu, biMinimize]
  BorderStyle = bsSingle
  ClientHeight = 649
  ClientWidth = 757
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  Position = poScreenCenter
  OnCreate = FormCreate
  OnKeyDown = FormKeyDown
  OnKeyPress = FormKeyPress
  PixelsPerInch = 96
  TextHeight = 13
  object imgNovo: TImage
    Left = 692
    Top = 39
    Width = 51
    Height = 44
    Cursor = crHandPoint
    Hint = 'Incluir'
    ParentShowHint = False
    Picture.Data = {
      
      426082}
    ShowHint = True
    Stretch = True
    OnClick = actIncluirExecute
  end
  object lblF2: TLabel
    Left = 705
    Top = 46
    Width = 12
    Height = 13
    Cursor = crHandPoint
    Hint = 'Incluir'
    Caption = 'F2'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlack
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    OnClick = actIncluirExecute
  end
  object imgDesfazer: TImage
    Left = 692
    Top = 129
    Width = 51
    Height = 44
    Cursor = crHandPoint
    Hint = 'Desfazer'
    ParentShowHint = False
    Picture.Data = {
      
      4E44AE426082}
    ShowHint = True
    Stretch = True
  end
  object lblF3: TLabel
    Left = 705
    Top = 132
    Width = 12
    Height = 13
    Cursor = crHandPoint
    Hint = 'Desfazer'
    Caption = 'F3'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWhite
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
  end
  object imgSalvar: TImage
    Left = 692
    Top = 219
    Width = 51
    Height = 44
    Cursor = crHandPoint
    Hint = 'Salvar'
    ParentShowHint = False
    Picture.Data = {
      
      AE426082}
    ShowHint = True
    Stretch = True
    OnClick = actSalvarExecute
  end
  object lblF4: TLabel
    Left = 710
    Top = 229
    Width = 12
    Height = 13
    Cursor = crHandPoint
    Hint = 'Salvar'
    Caption = 'F4'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlack
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    OnClick = actSalvarExecute
  end
  object imgExcluir: TImage
    Left = 692
    Top = 309
    Width = 51
    Height = 44
    Cursor = crHandPoint
    Hint = 'Excluir'
    ParentShowHint = False
    Picture.Data = {
      
      6082}
    ShowHint = True
    Stretch = True
    OnClick = lblF5Click
  end
  object lblF5: TLabel
    Left = 708
    Top = 319
    Width = 12
    Height = 13
    Cursor = crHandPoint
    Hint = 'Excluir'
    Caption = 'F5'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlack
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    OnClick = lblF5Click
  end
  object imgEditar: TImage
    Left = 692
    Top = 400
    Width = 51
    Height = 44
    Cursor = crHandPoint
    Hint = 'Editar'
    ParentShowHint = False
    Picture.Data = {
      
      82}
    ShowHint = True
    Stretch = True
    OnClick = actEditarExecute
  end
  object lblF6: TLabel
    Left = 710
    Top = 411
    Width = 12
    Height = 13
    Cursor = crHandPoint
    Hint = 'Editar'
    Caption = 'F6'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlack
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    OnClick = actEditarExecute
  end
  object imgSair: TImage
    Left = 692
    Top = 600
    Width = 51
    Height = 44
    Cursor = crHandPoint
    Hint = 'Sair'
    ParentShowHint = False
    Picture.Data = {
      }
    ShowHint = True
    Stretch = True
    OnClick = WindowClose1Execute
  end
  object grpPesquisa: TGroupBox
    Left = 8
    Top = 10
    Width = 668
    Height = 70
    Caption = 'Pesquisa'
    TabOrder = 0
    object rdpOpcoesPesquisa: TRadioGroup
      Left = 8
      Top = 18
      Width = 289
      Height = 48
      Cursor = crHandPoint
      Caption = 'Op'#231#245'es'
      Columns = 2
      ItemIndex = 0
      Items.Strings = (
        'CFOP/CFPS'
        'Descri'#231#227'o')
      TabOrder = 0
      OnClick = rdpOpcoesPesquisaClick
    end
    object grpConteudoPesquisa: TGroupBox
      Left = 299
      Top = 19
      Width = 364
      Height = 48
      Caption = 'Conte'#250'do'
      TabOrder = 1
      object impPesquisar: TImage
        Left = 331
        Top = 19
        Width = 27
        Height = 22
        Cursor = crHandPoint
        Hint = 'pesquisar'
        ParentShowHint = False
        Picture.Data = {
          
          00000049454E44AE426082}
        ShowHint = True
        Stretch = True
        OnClick = edtConteudoPesquisaExit
      end
      object edtConteudoPesquisa: TEdit
        Left = 8
        Top = 20
        Width = 318
        Height = 21
        CharCase = ecUpperCase
        TabOrder = 0
        TextHint = 'Digite aqui para pesquisar, e pressione <ENTER>'
        OnEnter = edtConteudoPesquisaEnter
        OnExit = edtConteudoPesquisaExit
        OnKeyDown = edtConteudoPesquisaKeyDown
        OnKeyPress = edtConteudoPesquisaKeyPress
      end
    end
  end
  object grpMensagem: TGroupBox
    Left = 4
    Top = 607
    Width = 671
    Height = 37
    Caption = 'Mensagem'
    TabOrder = 1
    object lblMsg: TLabel
      Left = 66
      Top = 14
      Width = 600
      Height = 16
      ParentCustomHint = False
      AutoSize = False
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -12
      Font.Name = 'Tahoma'
      Font.Style = [fsItalic]
      ParentFont = False
    end
  end
  object pgcCFOP_CFPS: TPageControl
    Left = 8
    Top = 93
    Width = 672
    Height = 508
    ActivePage = tbsCadastro
    TabOrder = 2
    object tbsTabela: TTabSheet
      Caption = 'Tabela'
      object grdTabela: TDBGrid
        Left = 0
        Top = 0
        Width = 664
        Height = 480
        Align = alClient
        DataSource = dmMSource.dtsCFOP_CFPS
        DrawingStyle = gdsClassic
        Options = [dgTitles, dgIndicator, dgColLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit, dgTitleClick, dgTitleHotTrack]
        ReadOnly = True
        TabOrder = 0
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -11
        TitleFont.Name = 'Tahoma'
        TitleFont.Style = []
        OnDrawColumnCell = grdTabelaDrawColumnCell
        OnDblClick = grdTabelaDblClick
        OnKeyPress = grdTabelaKeyPress
        Columns = <
          item
            Expanded = False
            FieldName = 'CFOP'
            Title.Alignment = taCenter
            Width = 51
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'DESCRICAO'
            Title.Alignment = taCenter
            Title.Caption = 'DESCRI'#199#195'O'
            Width = 454
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'DESCRICAO_NATUREZA'
            Title.Alignment = taCenter
            Title.Caption = 'FINALIDADE'
            Width = 123
            Visible = True
          end>
      end
    end
    object tbsCadastro: TTabSheet
      Caption = 'Cadastro'
      ImageIndex = 1
      object grpCadastro: TGroupBox
        Left = 0
        Top = 0
        Width = 664
        Height = 480
        Align = alClient
        Enabled = False
        TabOrder = 0
        object lblCFOP_CFPS: TLabel
          Left = 24
          Top = 12
          Width = 60
          Height = 13
          Caption = 'CFOP/CFPS:'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clRed
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
        end
        object lblDescricaoCFOP_CFPS: TLabel
          Left = 24
          Top = 35
          Width = 50
          Height = 13
          Caption = 'Descri'#231#227'o:'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clRed
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
        end
        object lblContaContabil: TLabel
          Left = 19
          Top = 340
          Width = 139
          Height = 13
          Caption = 'Conta cont'#225'bil (sped contrib)'
        end
        object edtCFOP_CFPS: TDBEdit
          Left = 93
          Top = 9
          Width = 69
          Height = 21
          DataField = 'CFOP'
          DataSource = dmMSource.dtsCFOP_CFPS
          MaxLength = 4
          TabOrder = 0
          OnEnter = edtCFOP_CFPSEnter
          OnExit = edtCFOP_CFPSExit
          OnKeyDown = edtConteudoPesquisaKeyDown
          OnKeyPress = edtCFOP_CFPSKeyPress
        end
        object edtDescricaoCFOP_CFPS: TDBEdit
          Left = 93
          Top = 32
          Width = 560
          Height = 21
          CharCase = ecUpperCase
          DataField = 'DESCRICAO'
          DataSource = dmMSource.dtsCFOP_CFPS
          TabOrder = 1
          OnEnter = edtCFOP_CFPSEnter
          OnExit = edtDescricaoCFOP_CFPSExit
          OnKeyDown = edtConteudoPesquisaKeyDown
          OnKeyPress = edtConteudoPesquisaKeyPress
        end
        object grpCorrelatas: TGroupBox
          Left = 19
          Top = 66
          Width = 171
          Height = 89
          Caption = 'Correlatas - entradas'
          TabOrder = 2
          object lblCorrelataEstadual: TLabel
            Left = 8
            Top = 21
            Width = 41
            Height = 13
            Caption = 'Estadual'
          end
          object lblCorrelataInterEstadual: TLabel
            Left = 8
            Top = 43
            Width = 69
            Height = 13
            Caption = 'Inter-estadual'
          end
          object lblCorrelataEstrangeira: TLabel
            Left = 8
            Top = 65
            Width = 55
            Height = 13
            Caption = 'Estrangeira'
          end
          object edtCorrelataEstadual: TDBEdit
            Left = 83
            Top = 18
            Width = 72
            Height = 21
            DataField = 'CFOP_ENTRADA_ESTADUAL'
            DataSource = dmMSource.dtsCFOP_CFPS
            MaxLength = 4
            TabOrder = 0
            OnEnter = edtCFOP_CFPSEnter
            OnExit = edtCorrelataEstadualExit
            OnKeyDown = edtConteudoPesquisaKeyDown
            OnKeyPress = edtCFOP_CFPSKeyPress
          end
          object edtCorrelataInterEstadual: TDBEdit
            Left = 83
            Top = 40
            Width = 72
            Height = 21
            DataField = 'CFOP_ENTRADA_INTERESTADUAL'
            DataSource = dmMSource.dtsCFOP_CFPS
            MaxLength = 4
            TabOrder = 1
            OnEnter = edtCFOP_CFPSEnter
            OnExit = edtCorrelataInterEstadualExit
            OnKeyDown = edtConteudoPesquisaKeyDown
            OnKeyPress = edtCFOP_CFPSKeyPress
          end
          object edtCorrelataEstrangeira: TDBEdit
            Left = 83
            Top = 62
            Width = 72
            Height = 21
            DataField = 'CFOP_ENTRADA_ESTRANGEIRA'
            DataSource = dmMSource.dtsCFOP_CFPS
            MaxLength = 4
            TabOrder = 2
            OnEnter = edtCFOP_CFPSEnter
            OnExit = edtCorrelataEstrangeiraExit
            OnKeyDown = edtConteudoPesquisaKeyDown
            OnKeyPress = edtCFOP_CFPSKeyPress
          end
        end
        object rdgOperacaoEstoque: TDBRadioGroup
          Left = 193
          Top = 70
          Width = 236
          Height = 85
          Caption = 'Opera'#231#227'o com estoque'
          Columns = 2
          DataField = 'OPERACAO'
          DataSource = dmMSource.dtsCFOP_CFPS
          Items.Strings = (
            'Entrada'
            'Sa'#237'da'
            'Implanta'#231#227'o'
            'Sem movimenta'#231#227'o')
          TabOrder = 3
          Values.Strings = (
            '0'
            '1'
            '2'
            '3')
          OnClick = rdgOperacaoEstoqueClick
        end
        object rdgFinalidade: TDBRadioGroup
          Left = 433
          Top = 66
          Width = 220
          Height = 89
          Caption = 'Finalidade'
          Columns = 2
          DataField = 'NATUREZA_CFOP'
          DataSource = dmMSource.dtsCFOP_CFPS
          Items.Strings = (
            'VENDA'
            'COMPRA'
            'DEVOLUCAO'
            'FRETE'
            'SERVI'#199'O'
            'CONSUMO'
            'OUTROS'
            'PRODU'#199#195'O')
          TabOrder = 4
          Values.Strings = (
            '0'
            '1'
            '2'
            '3'
            '4'
            '5'
            '6'
            '7')
          OnClick = rdgFinalidadeClick
        end
        object chkAtualizaCustos: TDBCheckBox
          Left = 19
          Top = 159
          Width = 155
          Height = 17
          Caption = 'Atualiza custos na entrada'
          DataField = 'ATUALIZAR_CUSTOS'
          DataSource = dmMSource.dtsCFOP_CFPS
          TabOrder = 5
          ValueChecked = '1'
          ValueUnchecked = '0'
        end
        object chkGeraCredDebICMS: TDBCheckBox
          Left = 19
          Top = 179
          Width = 155
          Height = 17
          Caption = 'Gera cr'#233'dito/d'#233'bito de ICMS'
          DataField = 'GERA_DEBITO_CREDITO_ICMS'
          DataSource = dmMSource.dtsCFOP_CFPS
          TabOrder = 6
          ValueChecked = '1'
          ValueUnchecked = '0'
        end
        object chkSubtsTrib: TDBCheckBox
          Left = 19
          Top = 199
          Width = 198
          Height = 17
          Caption = 'CFOP sujeita a subtitui'#231#227'o  tribut'#225'ria'
          DataField = 'SUBSTITUICAO_TRIBUTARIA'
          DataSource = dmMSource.dtsCFOP_CFPS
          TabOrder = 7
          ValueChecked = '1'
          ValueUnchecked = '0'
        end
        object chkGeraCredDebIPI: TDBCheckBox
          Left = 19
          Top = 217
          Width = 134
          Height = 17
          Caption = 'Gera d'#233'bito/cr'#233'dito IPI'
          DataField = 'GERA_DEBITO_CREDITO_IPI'
          DataSource = dmMSource.dtsCFOP_CFPS
          TabOrder = 8
          ValueChecked = '1'
          ValueUnchecked = '0'
          OnClick = chkGeraCredDebIPIClick
        end
        object chkGeraCREDDebPIS_COFINS: TDBCheckBox
          Left = 19
          Top = 237
          Width = 171
          Height = 17
          Caption = 'Gera cr'#233'dito/d'#233'bito PIS/COFINS'
          DataField = 'GERA_CREDITO_PIS_COFINS'
          DataSource = dmMSource.dtsCFOP_CFPS
          TabOrder = 9
          ValueChecked = '1'
          ValueUnchecked = '0'
          OnClick = chkGeraCREDDebPIS_COFINSClick
        end
        object frpObservacaoNFS: TGroupBox
          Left = 217
          Top = 159
          Width = 436
          Height = 121
          Caption = 'Observa'#231#227'o - NFS'
          TabOrder = 10
          object memObservacao: TDBMemo
            Left = 2
            Top = 15
            Width = 432
            Height = 104
            Align = alClient
            DataField = 'OBSERVACAO_NFS'
            DataSource = dmMSource.dtsCFOP_CFPS
            TabOrder = 0
            OnExit = memObservacaoExit
          end
        end
        object grpNaturezaReceita: TGroupBox
          Left = 19
          Top = 283
          Width = 632
          Height = 48
          Caption = 'Natureza da base de c'#225'lculo PIS/COFINS'
          TabOrder = 11
          object lblNaturezaPIS_COFINS: TLabel
            Left = 9
            Top = 22
            Width = 33
            Height = 13
            Caption = 'C'#243'digo'
          end
          object edtNaturezaPIS_COFINS: TDBEdit
            Left = 56
            Top = 19
            Width = 53
            Height = 21
            DataField = 'TABELA_4_3_7'
            DataSource = dmMSource.dtsCFOP_CFPS
            MaxLength = 2
            TabOrder = 0
            OnEnter = edtCFOP_CFPSEnter
            OnExit = edtNaturezaPIS_COFINSExit
            OnKeyDown = edtConteudoPesquisaKeyDown
            OnKeyPress = edtCFOP_CFPSKeyPress
          end
          object pnlNaturezaReceita: TPanel
            Left = 112
            Top = 17
            Width = 503
            Height = 28
            Alignment = taLeftJustify
            BevelOuter = bvNone
            BevelWidth = 2
            BorderWidth = 1
            Enabled = False
            TabOrder = 1
            object edtDescricaoNaturezaReceita: TDBEdit
              Left = 5
              Top = 2
              Width = 492
              Height = 21
              DataField = 'DESCRICAO_NATUREZA_RECEITA'
              DataSource = dmMSource.dtsCFOP_CFPS
              TabOrder = 0
            end
          end
        end
        object pgcContribuicoes: TPageControl
          Left = 19
          Top = 376
          Width = 632
          Height = 100
          ActivePage = tbsPIS_COFINS_Entrada
          TabOrder = 13
          object tbsPIS_COFINS_Entrada: TTabSheet
            Caption = 'PIS/COFINS entrada'
            object grpPIS_Entrada: TGroupBox
              Left = 2
              Top = 1
              Width = 218
              Height = 70
              Caption = 'PIS'
              TabOrder = 0
              object lblCST_PIS_Entrada: TLabel
                Left = 24
                Top = 20
                Width = 19
                Height = 13
                Caption = 'CST'
              end
              object lblAliquota_PIS_Entrada: TLabel
                Left = 24
                Top = 44
                Width = 39
                Height = 13
                Caption = 'Al'#237'quota'
              end
              object edtCST_PIS_Entrada: TDBEdit
                Left = 72
                Top = 17
                Width = 40
                Height = 21
                DataField = 'CST_PIS_ENTRADA'
                DataSource = dmMSource.dtsCFOP_CFPS
                MaxLength = 2
                TabOrder = 0
                OnEnter = edtCFOP_CFPSEnter
                OnExit = edtCST_PIS_EntradaExit
                OnKeyDown = edtConteudoPesquisaKeyDown
                OnKeyPress = edtCFOP_CFPSKeyPress
              end
              object edtAliquotaCST_PIS_Entrada: TDBEdit
                Left = 72
                Top = 41
                Width = 77
                Height = 21
                DataField = 'ALIQUOTA_PIS_ENTRADA'
                DataSource = dmMSource.dtsCFOP_CFPS
                MaxLength = 5
                TabOrder = 1
                OnEnter = edtCFOP_CFPSEnter
                OnExit = edtAliquotaCST_PIS_EntradaExit
                OnKeyDown = edtConteudoPesquisaKeyDown
                OnKeyPress = edtALIQUOTA_PIS_SAIDAKeyPress
              end
            end
            object grpCOFINS_Entrada: TGroupBox
              Left = 225
              Top = 1
              Width = 218
              Height = 70
              Caption = 'COFINS'
              TabOrder = 1
              object lblCST_COFINS_Entrada: TLabel
                Left = 32
                Top = 18
                Width = 19
                Height = 13
                Caption = 'CST'
              end
              object lblAliquota_COFINS_Entrada: TLabel
                Left = 32
                Top = 42
                Width = 39
                Height = 13
                Caption = 'Al'#237'quota'
              end
              object edtCST_COFINS_Entrada: TDBEdit
                Left = 79
                Top = 15
                Width = 40
                Height = 21
                DataField = 'CST_COFINS_ENTRADA'
                DataSource = dmMSource.dtsCFOP_CFPS
                MaxLength = 2
                TabOrder = 0
                OnEnter = edtCFOP_CFPSEnter
                OnExit = edtCST_COFINS_EntradaExit
                OnKeyDown = edtConteudoPesquisaKeyDown
                OnKeyPress = edtCFOP_CFPSKeyPress
              end
              object edtAliquotaCST_COFINS_Entrada: TDBEdit
                Left = 79
                Top = 39
                Width = 77
                Height = 21
                DataField = 'ALIQUOTA_COFINS_ENTRADA'
                DataSource = dmMSource.dtsCFOP_CFPS
                MaxLength = 5
                TabOrder = 1
                OnEnter = edtCFOP_CFPSEnter
                OnExit = edtAliquotaCST_COFINS_EntradaExit
                OnKeyDown = edtConteudoPesquisaKeyDown
                OnKeyPress = edtALIQUOTA_PIS_SAIDAKeyPress
              end
            end
            object GroupBox2: TGroupBox
              Left = 445
              Top = 1
              Width = 130
              Height = 70
              Caption = 'C'#243'digo do cr'#233'dito'
              TabOrder = 2
              object edtCodigoCreditoPisCofins: TDBEdit
                Left = 44
                Top = 32
                Width = 40
                Height = 21
                DataField = 'CODIGO_CREDITO_PIS_COFINS'
                DataSource = dmMSource.dtsCFOP_CFPS
                MaxLength = 3
                TabOrder = 0
                OnEnter = edtCFOP_CFPSEnter
                OnExit = edtNaturezaReceitaExit
                OnKeyDown = edtConteudoPesquisaKeyDown
                OnKeyPress = edtCFOP_CFPSKeyPress
              end
            end
          end
          object tbsPIS_COFINS_Saida: TTabSheet
            Caption = 'PIS/COFINS sa'#237'da'
            ImageIndex = 1
            object grpPIS_Saida: TGroupBox
              Left = 2
              Top = 1
              Width = 218
              Height = 70
              Caption = 'PIS'
              TabOrder = 0
              object lblCST_PIS_Saida: TLabel
                Left = 24
                Top = 18
                Width = 19
                Height = 13
                Caption = 'CST'
              end
              object lblAliquota_PIS_Saida: TLabel
                Left = 24
                Top = 42
                Width = 39
                Height = 13
                Caption = 'Al'#237'quota'
              end
              object edtCST_PIS_Saida: TDBEdit
                Left = 69
                Top = 15
                Width = 40
                Height = 21
                DataField = 'CST_PIS_SAIDA'
                DataSource = dmMSource.dtsCFOP_CFPS
                MaxLength = 2
                TabOrder = 0
                OnEnter = edtCFOP_CFPSEnter
                OnExit = edtCST_PIS_SaidaExit
                OnKeyDown = edtConteudoPesquisaKeyDown
                OnKeyPress = edtCFOP_CFPSKeyPress
              end
              object edtALIQUOTA_PIS_SAIDA: TDBEdit
                Left = 69
                Top = 39
                Width = 77
                Height = 21
                DataField = 'ALIQUOTA_PIS_SAIDA'
                DataSource = dmMSource.dtsCFOP_CFPS
                MaxLength = 5
                TabOrder = 1
                OnEnter = edtCFOP_CFPSEnter
                OnExit = edtALIQUOTA_PIS_SAIDAExit
                OnKeyDown = edtConteudoPesquisaKeyDown
                OnKeyPress = edtALIQUOTA_PIS_SAIDAKeyPress
              end
            end
            object grpCOFINS_Saida: TGroupBox
              Left = 225
              Top = 1
              Width = 218
              Height = 70
              Caption = 'COFINS'
              TabOrder = 1
              object lblCST_COFINS_Saida: TLabel
                Left = 24
                Top = 18
                Width = 19
                Height = 13
                Caption = 'CST'
              end
              object lblAliquota_COFINS_Saida: TLabel
                Left = 24
                Top = 42
                Width = 39
                Height = 13
                Caption = 'Al'#237'quota'
              end
              object edtCST_COFINS_SAIDA: TDBEdit
                Left = 74
                Top = 15
                Width = 40
                Height = 21
                DataField = 'CST_COFINS_SAIDA'
                DataSource = dmMSource.dtsCFOP_CFPS
                MaxLength = 2
                TabOrder = 0
                OnEnter = edtCFOP_CFPSEnter
                OnExit = edtCST_COFINS_SAIDAExit
                OnKeyDown = edtConteudoPesquisaKeyDown
                OnKeyPress = edtCFOP_CFPSKeyPress
              end
              object edtALIQUOTA_COFINS_SAIDA: TDBEdit
                Left = 74
                Top = 39
                Width = 77
                Height = 21
                DataField = 'ALIQUOTA_COFINS_SAIDA'
                DataSource = dmMSource.dtsCFOP_CFPS
                MaxLength = 5
                TabOrder = 1
                OnEnter = edtCFOP_CFPSEnter
                OnExit = edtALIQUOTA_COFINS_SAIDAExit
                OnKeyDown = edtConteudoPesquisaKeyDown
                OnKeyPress = edtALIQUOTA_PIS_SAIDAKeyPress
              end
            end
            object GroupBox1: TGroupBox
              Left = 445
              Top = 1
              Width = 130
              Height = 70
              Caption = 'Natureza da receita'
              TabOrder = 2
              object edtNaturezaReceita: TDBEdit
                Left = 44
                Top = 32
                Width = 40
                Height = 21
                DataField = 'NATUREZA_RECEITA_PIS_COFINS'
                DataSource = dmMSource.dtsCFOP_CFPS
                MaxLength = 3
                TabOrder = 0
                OnEnter = edtCFOP_CFPSEnter
                OnExit = edtNaturezaReceitaExit
                OnKeyDown = edtConteudoPesquisaKeyDown
                OnKeyPress = edtCFOP_CFPSKeyPress
              end
            end
          end
        end
        object chkCFOPRemessa: TDBCheckBox
          Left = 19
          Top = 257
          Width = 171
          Height = 17
          Caption = 'CFOP para remessa'
          DataField = 'REMESSA'
          DataSource = dmMSource.dtsCFOP_CFPS
          TabOrder = 14
          ValueChecked = '1'
          ValueUnchecked = '0'
        end
        object edtCod_Cta: TDBEdit
          Left = 168
          Top = 337
          Width = 460
          Height = 21
          DataField = 'COD_CTA'
          DataSource = dmMSource.dtsCFOP_CFPS
          MaxLength = 255
          TabOrder = 12
          OnEnter = edtCFOP_CFPSEnter
          OnExit = edtCod_CtaExit
          OnKeyDown = edtConteudoPesquisaKeyDown
          OnKeyPress = edtCod_CtaKeyPress
        end
      end
    end
  end
  object ActionList1: TActionList
    Left = 372
    Top = 292
    object WindowClose1: TWindowClose
      Category = 'Window'
      Caption = 'C&lose'
      Enabled = False
      Hint = 'Close'
      OnExecute = WindowClose1Execute
    end
    object actIncluir: TAction
      Category = 'Window'
      Caption = 'actIncluir'
      ShortCut = 113
      OnExecute = actIncluirExecute
    end
    object actDesfazer: TAction
      Category = 'Window'
      Caption = 'actDesfazer'
      ShortCut = 114
      OnExecute = actDesfazerExecute
    end
    object actSalvar: TAction
      Category = 'Window'
      Caption = 'actSalvar'
      ShortCut = 115
      OnExecute = actSalvarExecute
    end
    object actEditar: TAction
      Category = 'Window'
      Caption = 'actEditar'
      ShortCut = 117
      OnExecute = actEditarExecute
    end
    object actExcluir: TAction
      Category = 'Window'
      Caption = 'actExcluir'
      OnExecute = actExcluirExecute
    end
  end
end
