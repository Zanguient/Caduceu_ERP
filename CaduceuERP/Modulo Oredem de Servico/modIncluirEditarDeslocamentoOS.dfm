object frmIncluirEditarDeslocamentoOS: TfrmIncluirEditarDeslocamentoOS
  Left = 0
  Top = 0
  BorderIcons = [biSystemMenu, biMinimize]
  BorderStyle = bsSingle
  Caption = 'Incluir/Editar Deslocamento'
  ClientHeight = 445
  ClientWidth = 510
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  Position = poScreenCenter
  OnCreate = FormCreate
  OnKeyDown = FormKeyDown
  PixelsPerInch = 96
  TextHeight = 13
  object imgDesfazer: TImage
    Left = 451
    Top = 18
    Width = 51
    Height = 44
    Cursor = crHandPoint
    Hint = 'Desfazer'
    ParentShowHint = False
    Picture.Data = {
      
      4E44AE426082}
    ShowHint = True
    Stretch = True
    OnClick = lblF3Click
  end
  object imgSair: TImage
    Left = 451
    Top = 388
    Width = 51
    Height = 44
    Cursor = crHandPoint
    Hint = 'Sair'
    ParentShowHint = False
    Picture.Data = {
      }
    ShowHint = True
    Stretch = True
    OnClick = imgSairClick
  end
  object imgSalvar: TImage
    Left = 451
    Top = 109
    Width = 51
    Height = 44
    Cursor = crHandPoint
    Hint = 'Salvar'
    ParentShowHint = False
    Picture.Data = {
      
      AE426082}
    ShowHint = True
    Stretch = True
    OnClick = lblF4Click
  end
  object lblF4: TLabel
    Left = 471
    Top = 119
    Width = 12
    Height = 13
    Cursor = crHandPoint
    Hint = 'Salvar'
    Caption = 'F4'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlack
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    OnClick = lblF4Click
  end
  object lblF3: TLabel
    Left = 471
    Top = 30
    Width = 12
    Height = 13
    Cursor = crHandPoint
    Hint = 'Desfazer'
    Caption = 'F3'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWhite
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    OnClick = lblF3Click
  end
  object grpOrigemsaida: TGroupBox
    Left = 8
    Top = 73
    Width = 431
    Height = 67
    Caption = 'Sa'#237'da - Origem'
    TabOrder = 1
    object lblHorarioSaidaOrigem: TLabel
      Left = 23
      Top = 28
      Width = 78
      Height = 13
      Caption = 'Hor'#225'rio da sa'#237'da'
    end
    object lblKMSaidaOrigem: TLabel
      Left = 221
      Top = 28
      Width = 57
      Height = 13
      Caption = 'KM de sa'#237'da'
    end
    object tmrHoraSaidaOrigem: TJvDBDateTimePicker
      Left = 124
      Top = 25
      Width = 81
      Height = 21
      Date = 42696.000000000000000000
      Format = 'HH:mm'
      Time = 42696.000000000000000000
      Kind = dtkTime
      TabOrder = 0
      OnEnter = dtpDataSaidaOrigemEnter
      OnExit = dtpDataSaidaOrigemEnter
      OnKeyPress = edtAlimentacaoKeyPress
      DropDownDate = 42696.000000000000000000
      DataField = 'HORA_SAIDA_ORIGEM'
      DataSource = dmMSource.dtsDeslocamentoOS
    end
    object edtKMSaidaOrigem: TDBEdit
      Left = 302
      Top = 25
      Width = 107
      Height = 21
      DataField = 'KM_SAIDA_ORIGEM'
      DataSource = dmMSource.dtsDeslocamentoOS
      TabOrder = 1
      OnEnter = dtpDataSaidaOrigemEnter
      OnExit = dtpDataSaidaOrigemEnter
      OnKeyDown = dtpDataSaidaOrigemKeyDown
      OnKeyPress = edtAlimentacaoKeyPress
    end
  end
  object grpChegadaDestino: TGroupBox
    Left = 8
    Top = 146
    Width = 431
    Height = 67
    Caption = 'Chegada - Destino'
    TabOrder = 2
    object lblHorarioChegadaDestino: TLabel
      Left = 23
      Top = 28
      Width = 94
      Height = 13
      Caption = 'Hor'#225'rio da chegada'
    end
    object lblKMChegadaDestino: TLabel
      Left = 221
      Top = 28
      Width = 73
      Height = 13
      Caption = 'KM de chegada'
    end
    object tmrHoraChegadaDestino: TJvDBDateTimePicker
      Left = 124
      Top = 25
      Width = 81
      Height = 21
      Date = 42696.000000000000000000
      Format = 'HH:mm'
      Time = 42696.000000000000000000
      Kind = dtkTime
      TabOrder = 0
      OnEnter = dtpDataSaidaOrigemEnter
      OnExit = dtpDataSaidaOrigemEnter
      OnKeyPress = edtAlimentacaoKeyPress
      DropDownDate = 42696.000000000000000000
      DataField = 'HORA_CHEGADA_DESTINO'
      DataSource = dmMSource.dtsDeslocamentoOS
    end
    object edtKMChegadaDestino: TDBEdit
      Left = 302
      Top = 25
      Width = 107
      Height = 21
      DataField = 'KM_CHEGADA_DESTINO'
      DataSource = dmMSource.dtsDeslocamentoOS
      TabOrder = 1
      OnEnter = dtpDataSaidaOrigemEnter
      OnExit = dtpDataSaidaOrigemEnter
      OnKeyDown = dtpDataSaidaOrigemKeyDown
      OnKeyPress = edtAlimentacaoKeyPress
    end
  end
  object grpSaidaDestino: TGroupBox
    Left = 8
    Top = 219
    Width = 431
    Height = 67
    Caption = 'Sa'#237'da - Destino'
    TabOrder = 3
    object lblHorarioSaidaDestino: TLabel
      Left = 23
      Top = 31
      Width = 78
      Height = 13
      Caption = 'Hor'#225'rio da sa'#237'da'
    end
    object lblKMSaidaDestino: TLabel
      Left = 221
      Top = 31
      Width = 57
      Height = 13
      Caption = 'KM de sa'#237'da'
    end
    object tmrHorarioSaidaDestino: TJvDBDateTimePicker
      Left = 124
      Top = 28
      Width = 81
      Height = 21
      Date = 42696.000000000000000000
      Format = 'HH:mm'
      Time = 42696.000000000000000000
      Kind = dtkTime
      TabOrder = 0
      OnEnter = dtpDataSaidaOrigemEnter
      OnExit = dtpDataSaidaOrigemEnter
      OnKeyPress = edtAlimentacaoKeyPress
      DropDownDate = 42696.000000000000000000
      DataField = 'HORA_SAIDA_DESTINO'
      DataSource = dmMSource.dtsDeslocamentoOS
    end
    object edtKMSaidaDestino: TDBEdit
      Left = 302
      Top = 28
      Width = 107
      Height = 21
      DataField = 'KM_SAIDA_DESTINO'
      DataSource = dmMSource.dtsDeslocamentoOS
      TabOrder = 1
      OnEnter = dtpDataSaidaOrigemEnter
      OnExit = dtpDataSaidaOrigemEnter
      OnKeyDown = dtpDataSaidaOrigemKeyDown
      OnKeyPress = edtAlimentacaoKeyPress
    end
  end
  object grpRegressoOrigem: TGroupBox
    Left = 8
    Top = 292
    Width = 431
    Height = 67
    Caption = 'Regresso - Origem'
    TabOrder = 4
    object lblHorarioRegressoOrigem: TLabel
      Left = 23
      Top = 28
      Width = 95
      Height = 13
      Caption = 'Hor'#225'rio do regresso'
    end
    object lblKMRegressoOrigem: TLabel
      Left = 221
      Top = 28
      Width = 74
      Height = 13
      Caption = 'KM do regresso'
    end
    object tmrHorarioRegressoOrigem: TJvDBDateTimePicker
      Left = 124
      Top = 25
      Width = 81
      Height = 21
      Date = 42696.000000000000000000
      Format = 'HH:mm'
      Time = 42696.000000000000000000
      Kind = dtkTime
      TabOrder = 0
      OnEnter = dtpDataSaidaOrigemEnter
      OnExit = dtpDataSaidaOrigemEnter
      OnKeyPress = edtAlimentacaoKeyPress
      DropDownDate = 42696.000000000000000000
      DataField = 'HORA_REGRESSO_ORIGEM'
      DataSource = dmMSource.dtsDeslocamentoOS
    end
    object edtRegressoOrigem: TDBEdit
      Left = 302
      Top = 25
      Width = 107
      Height = 21
      DataField = 'KM_REGRESSO_ORIGEM'
      DataSource = dmMSource.dtsDeslocamentoOS
      TabOrder = 1
      OnEnter = dtpDataSaidaOrigemEnter
      OnExit = dtpDataSaidaOrigemEnter
      OnKeyDown = dtpDataSaidaOrigemKeyDown
      OnKeyPress = edtAlimentacaoKeyPress
    end
  end
  object grpDespesas: TGroupBox
    Left = 8
    Top = 365
    Width = 431
    Height = 67
    Caption = 'Despesas'
    TabOrder = 5
    object lblAlimentacao: TLabel
      Left = 4
      Top = 28
      Width = 82
      Height = 13
      Caption = 'Alimenta'#231#227'o (R$)'
    end
    object lblHospedagem: TLabel
      Left = 207
      Top = 28
      Width = 86
      Height = 13
      Caption = 'Hospedagem (R$)'
    end
    object edtHospedagem: TDBEdit
      Left = 314
      Top = 25
      Width = 107
      Height = 21
      DataField = 'VALOR_HOSPEDAGEM'
      DataSource = dmMSource.dtsDeslocamentoOS
      TabOrder = 1
      OnEnter = dtpDataSaidaOrigemEnter
      OnExit = dtpDataSaidaOrigemEnter
      OnKeyDown = dtpDataSaidaOrigemKeyDown
      OnKeyPress = edtAlimentacaoKeyPress
    end
    object edtAlimentacao: TDBEdit
      Left = 92
      Top = 25
      Width = 107
      Height = 21
      DataField = 'VALOR_ALIMENTACAO'
      DataSource = dmMSource.dtsDeslocamentoOS
      TabOrder = 0
      OnEnter = dtpDataSaidaOrigemEnter
      OnExit = dtpDataSaidaOrigemEnter
      OnKeyDown = dtpDataSaidaOrigemKeyDown
      OnKeyPress = edtAlimentacaoKeyPress
    end
  end
  object grpData: TGroupBox
    Left = 8
    Top = 8
    Width = 169
    Height = 62
    Caption = 'Data'
    TabOrder = 0
    object dtpDataSaidaOrigem: TJvDBDatePickerEdit
      Left = 32
      Top = 24
      Width = 105
      Height = 21
      AllowNoDate = True
      DataField = 'DATA_CADASTRO'
      DataSource = dmMSource.dtsDeslocamentoOS
      TabOrder = 0
      OnEnter = dtpDataSaidaOrigemEnter
      OnExit = dtpDataSaidaOrigemEnter
      OnKeyDown = dtpDataSaidaOrigemKeyDown
      OnKeyPress = dtpDataSaidaOrigemKeyPress
    end
  end
end
