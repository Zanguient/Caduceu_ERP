...
type
  TfrmItemServicoAvulso = class(TForm)
    ...
    procedure memServicoKeyPress(Sender: TObject; var Key: Char);
  private
...
procedure TfrmItemServicoAvulso.imgConfirmarClick(Sender: TObject);
begin

  if Length(trim(dmMProvider.cdsItens_OS_servicoDESCRICAO.Value)) > 0 then
  begin
...
procedure TfrmItemServicoAvulso.memServicoKeyPress(Sender: TObject; var Key: Char);
begin
