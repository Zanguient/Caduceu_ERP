...
procedure TfrmDescontoEncerramentoOS.edtDescontoKeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin

  if (Shift = [ssShift]) and (Chr(Key) = '5') and (edtDesconto.Value > 0) then
  begin

    edtDesconto.Value     := dmMProvider.cdsOrdemServicoVALOR_TOTAL_OS.Value * (edtDesconto.Value / 100);
    edtValorTotalOS.Value := dmMProvider.cdsOrdemServicoVALOR_TOTAL_OS.Value - edtDesconto.Value;
