object frmEstados: TfrmEstados
  Left = 0
  Top = 0
  ActiveControl = edtConteudoPesquisa
  BorderIcons = [biSystemMenu, biMinimize]
  BorderStyle = bsSingle
  ClientHeight = 524
  ClientWidth = 742
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  Position = poScreenCenter
  OnCreate = FormCreate
  OnKeyDown = FormKeyDown
  OnKeyPress = FormKeyPress
  PixelsPerInch = 96
  TextHeight = 13
  object imgSair: TImage
    Left = 683
    Top = 476
    Width = 51
    Height = 44
    Cursor = crHandPoint
    Hint = 'Sair'
    ParentShowHint = False
    Picture.Data = {
      }
    ShowHint = True
    Stretch = True
    OnClick = WindowClose1Execute
  end
  object imgNovo: TImage
    Left = 683
    Top = 37
    Width = 51
    Height = 44
    Cursor = crHandPoint
    Hint = 'Incluir'
    ParentShowHint = False
    Picture.Data = {
      
      426082}
    ShowHint = True
    Stretch = True
    OnClick = actIncluirExecute
  end
  object imgSalvar: TImage
    Left = 683
    Top = 165
    Width = 51
    Height = 44
    Cursor = crHandPoint
    Hint = 'Salvar'
    ParentShowHint = False
    Picture.Data = {
      
      AE426082}
    ShowHint = True
    Stretch = True
    OnClick = actSalvarExecute
  end
  object lblF2: TLabel
    Left = 696
    Top = 53
    Width = 12
    Height = 13
    Cursor = crHandPoint
    Hint = 'Incluir'
    Caption = 'F2'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlack
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    OnClick = actIncluirExecute
  end
  object imgDesfazer: TImage
    Left = 683
    Top = 100
    Width = 51
    Height = 44
    Cursor = crHandPoint
    Hint = 'Desfazer'
    ParentShowHint = False
    Picture.Data = {
      
      4E44AE426082}
    ShowHint = True
    Stretch = True
    OnClick = actDesfazerExecute
  end
  object lblF3: TLabel
    Left = 696
    Top = 102
    Width = 12
    Height = 13
    Cursor = crHandPoint
    Hint = 'Desfazer'
    Caption = 'F3'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWhite
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    OnClick = actDesfazerExecute
  end
  object lblF4: TLabel
    Left = 701
    Top = 173
    Width = 12
    Height = 13
    Cursor = crHandPoint
    Hint = 'Salvar'
    Caption = 'F4'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlack
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    OnClick = actSalvarExecute
  end
  object imgExcluir: TImage
    Left = 683
    Top = 227
    Width = 51
    Height = 44
    Cursor = crHandPoint
    Hint = 'Excluir'
    ParentShowHint = False
    Picture.Data = {
      
      6082}
    ShowHint = True
    Stretch = True
    OnClick = lblF5Click
  end
  object lblF5: TLabel
    Left = 699
    Top = 240
    Width = 12
    Height = 13
    Cursor = crHandPoint
    Hint = 'Excluir'
    Caption = 'F5'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlack
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    OnClick = lblF5Click
  end
  object imgEditar: TImage
    Left = 683
    Top = 291
    Width = 51
    Height = 44
    Cursor = crHandPoint
    Hint = 'Editar'
    ParentShowHint = False
    Picture.Data = {
      
      82}
    ShowHint = True
    Stretch = True
    OnClick = actEditarExecute
  end
  object lblF6: TLabel
    Left = 701
    Top = 307
    Width = 12
    Height = 13
    Cursor = crHandPoint
    Hint = 'Editar'
    Caption = 'F6'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlack
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    OnClick = actEditarExecute
  end
  object grpMensagem: TGroupBox
    Left = 4
    Top = 487
    Width = 671
    Height = 33
    Caption = 'Mensagem'
    TabOrder = 0
    object lblMsg: TLabel
      Left = 66
      Top = 14
      Width = 600
      Height = 16
      ParentCustomHint = False
      AutoSize = False
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -12
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
    end
  end
  object pgcEstados: TPageControl
    Left = 8
    Top = 91
    Width = 667
    Height = 394
    ActivePage = TabSheet1
    TabOrder = 1
    object tbsTabela: TTabSheet
      Caption = 'Tabela'
      ExplicitHeight = 296
      object grdTabela: TDBGrid
        Left = 0
        Top = 0
        Width = 659
        Height = 366
        Cursor = crHandPoint
        Align = alClient
        DataSource = dmMSource.dtsEstados
        DrawingStyle = gdsClassic
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        Options = [dgTitles, dgIndicator, dgColLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit, dgTitleClick, dgTitleHotTrack]
        ParentFont = False
        ReadOnly = True
        TabOrder = 0
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -11
        TitleFont.Name = 'Tahoma'
        TitleFont.Style = []
        OnDrawColumnCell = grdTabelaDrawColumnCell
        OnDblClick = grdTabelaDblClick
        OnKeyPress = grdTabelaKeyPress
        OnTitleClick = grdTabelaTitleClick
        Columns = <
          item
            Expanded = False
            FieldName = 'SIGLA'
            Title.Alignment = taCenter
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'NOME'
            Title.Alignment = taCenter
            Width = 318
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'CODIGO_ESTADO_IBGE'
            Title.Alignment = taCenter
            Title.Caption = 'COD.IBGE'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'ICMS_INTERNO'
            Title.Alignment = taCenter
            Title.Caption = 'ICMS.EST.'
            Width = 70
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'ICMS_INTERESTADUAL'
            Title.Alignment = taCenter
            Title.Caption = 'ICMS INT.EST'
            Width = 70
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'MVA'
            Title.Alignment = taCenter
            Title.Caption = 'M.V.A'
            Visible = True
          end>
      end
    end
    object tdsCadastro: TTabSheet
      Caption = 'Cadastro'
      ImageIndex = 1
      ExplicitHeight = 296
      object grpCadastro: TGroupBox
        Left = 3
        Top = 2
        Width = 653
        Height = 245
        Enabled = False
        TabOrder = 0
        object lblSigla: TLabel
          Left = 16
          Top = 8
          Width = 22
          Height = 13
          Caption = 'Sigla'
          FocusControl = edtSigla
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clRed
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
        end
        object lblNome: TLabel
          Left = 16
          Top = 31
          Width = 27
          Height = 13
          Caption = 'Nome'
          FocusControl = edtNome
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clRed
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
        end
        object lblMVA: TLabel
          Left = 16
          Top = 146
          Width = 29
          Height = 13
          Caption = 'M.V.A'
          FocusControl = edtMVA
        end
        object lblCodigoIBGE: TLabel
          Left = 16
          Top = 54
          Width = 59
          Height = 13
          Caption = 'Codigo IBGE'
          FocusControl = edtCodigoIBGE
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clRed
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
        end
        object lblAliquota_ICMS_Interna: TLabel
          Left = 16
          Top = 100
          Width = 88
          Height = 13
          Caption = 'Aliq. ICMS Interna'
          FocusControl = edtAliqICMSInt
        end
        object lblAliquota_ICMS_InterEst: TLabel
          Left = 16
          Top = 123
          Width = 96
          Height = 13
          Caption = 'Aliq. ICMSInter.Est.'
          FocusControl = edtAliqICMSIntEst
        end
        object lblPercentualFCPPadrao: TLabel
          Left = 16
          Top = 169
          Width = 70
          Height = 13
          Caption = '% FCP padr'#227'o'
          FocusControl = grdFECP
        end
        object lblCodigoPais: TLabel
          Left = 15
          Top = 77
          Width = 59
          Height = 13
          Caption = 'Codigo PA'#205'S'
          FocusControl = edtCodigoPais
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clRed
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
        end
        object lblCodigoSINIEF: TLabel
          Left = 16
          Top = 214
          Width = 69
          Height = 13
          Caption = 'C'#243'digo SINIEF'
          FocusControl = edtCodigoSINIEF
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clRed
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
        end
        object Label1: TLabel
          Left = 371
          Top = 123
          Width = 90
          Height = 13
          Caption = 'M'#225'scara Insc.Est.:'
          FocusControl = edtMascaraInscEst
        end
        object lblMascaraAtivEst: TLabel
          Left = 371
          Top = 169
          Width = 89
          Height = 13
          Caption = 'M'#225'scara Ativ.Est.:'
          FocusControl = edtMascaraAtivEst
        end
        object lblAliquotaSimplesFederal: TLabel
          Left = 371
          Top = 214
          Width = 114
          Height = 13
          Caption = 'Aliquota Simples federal'
          FocusControl = edtAliquota_Simples_Federal
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
        end
        object edtSigla: TDBEdit
          Left = 129
          Top = 5
          Width = 48
          Height = 21
          CharCase = ecUpperCase
          DataField = 'SIGLA'
          DataSource = dmMSource.dtsEstados
          TabOrder = 0
          OnEnter = edtSiglaEnter
          OnExit = edtSiglaExit
          OnKeyDown = edtConteudoPesquisaKeyDown
          OnKeyPress = edtConteudoPesquisaKeyPress
        end
        object edtNome: TDBEdit
          Left = 129
          Top = 28
          Width = 512
          Height = 21
          CharCase = ecUpperCase
          DataField = 'NOME'
          DataSource = dmMSource.dtsEstados
          TabOrder = 1
          OnEnter = edtSiglaEnter
          OnExit = edtNomeExit
          OnKeyDown = edtConteudoPesquisaKeyDown
          OnKeyPress = edtConteudoPesquisaKeyPress
        end
        object edtCodigoIBGE: TDBEdit
          Left = 129
          Top = 51
          Width = 62
          Height = 21
          DataField = 'CODIGO_ESTADO_IBGE'
          DataSource = dmMSource.dtsEstados
          MaxLength = 2
          TabOrder = 2
          OnEnter = edtSiglaEnter
          OnExit = edtCodigoIBGEExit
          OnKeyDown = edtConteudoPesquisaKeyDown
          OnKeyPress = edtConteudoPesquisaKeyPress
        end
        object edtAliqICMSInt: TDBEdit
          Left = 129
          Top = 97
          Width = 62
          Height = 21
          DataField = 'ICMS_INTERNO'
          DataSource = dmMSource.dtsEstados
          MaxLength = 5
          TabOrder = 4
          OnEnter = edtSiglaEnter
          OnExit = edtAliqICMSIntExit
          OnKeyDown = edtConteudoPesquisaKeyDown
          OnKeyPress = edtAliqICMSIntKeyPress
        end
        object edtAliqICMSIntEst: TDBEdit
          Left = 129
          Top = 120
          Width = 62
          Height = 21
          DataField = 'ICMS_INTERESTADUAL'
          DataSource = dmMSource.dtsEstados
          MaxLength = 5
          TabOrder = 5
          OnEnter = edtSiglaEnter
          OnExit = edtAliqICMSIntEstExit
          OnKeyDown = edtConteudoPesquisaKeyDown
          OnKeyPress = edtAliqICMSIntKeyPress
        end
        object edtMVA: TDBEdit
          Left = 129
          Top = 143
          Width = 62
          Height = 21
          DataField = 'MVA'
          DataSource = dmMSource.dtsEstados
          MaxLength = 5
          TabOrder = 6
          OnEnter = edtSiglaEnter
          OnExit = edtMVAExit
          OnKeyDown = edtConteudoPesquisaKeyDown
          OnKeyPress = edtAliqICMSIntKeyPress
        end
        object edtPercentualFCPPadrao: TDBEdit
          Left = 129
          Top = 166
          Width = 62
          Height = 21
          DataField = 'FCP'
          DataSource = dmMSource.dtsEstados
          MaxLength = 5
          TabOrder = 7
          OnEnter = edtSiglaEnter
          OnExit = edtAliqICMSIntExit
          OnKeyDown = edtConteudoPesquisaKeyDown
          OnKeyPress = edtAliqICMSIntKeyPress
        end
        object chkEstadoBrasileiro: TDBCheckBox
          Left = 16
          Top = 190
          Width = 127
          Height = 17
          Alignment = taLeftJustify
          Caption = 'Estado brasileiro'
          DataField = 'ESTADO_BRASILEIRO'
          DataSource = dmMSource.dtsEstados
          TabOrder = 8
          ValueChecked = '1'
          ValueUnchecked = '0'
        end
        object edtCodigoPais: TDBEdit
          Left = 129
          Top = 74
          Width = 62
          Height = 21
          DataField = 'CODIGO_PAIS'
          DataSource = dmMSource.dtsEstados
          MaxLength = 2
          TabOrder = 3
          OnEnter = edtSiglaEnter
          OnExit = edtCodigoIBGEExit
          OnKeyDown = edtConteudoPesquisaKeyDown
          OnKeyPress = edtConteudoPesquisaKeyPress
        end
        object edtCodigoSINIEF: TDBEdit
          Left = 129
          Top = 211
          Width = 62
          Height = 21
          DataField = 'CODIGO_ESTADO_IBGE'
          DataSource = dmMSource.dtsEstados
          MaxLength = 2
          TabOrder = 9
          OnEnter = edtSiglaEnter
          OnExit = edtCodigoIBGEExit
          OnKeyDown = edtConteudoPesquisaKeyDown
          OnKeyPress = edtConteudoPesquisaKeyPress
        end
        object rdgRegiaoGeografica: TDBRadioGroup
          Left = 198
          Top = 51
          Width = 179
          Height = 67
          Caption = 'Regi'#227'o geogr'#225'fica'
          Columns = 2
          DataField = 'REGIAO_GEOGRAFICA'
          DataSource = dmMSource.dtsEstados
          Items.Strings = (
            'Norte'
            'Nordeste'
            'Sudeste'
            'Sul'
            'Centro Oeste'
            'Exterior')
          TabOrder = 14
          Values.Strings = (
            '1'
            '2'
            '3'
            '4'
            '5'
            '6')
        end
        object DBRadioGroup1: TDBRadioGroup
          Left = 378
          Top = 51
          Width = 263
          Height = 67
          Caption = 'Regi'#227'o Fiscal'
          DataField = 'REGIAO_FISCAL'
          DataSource = dmMSource.dtsEstados
          Items.Strings = (
            'Sul/Sudeste menos Espirito Santo'
            'Norte/Nordeste/Centro Oeste mais Espirito Santo'
            'Zona Franca de Manaus mais Exterior')
          TabOrder = 15
          Values.Strings = (
            '1'
            '2'
            '3')
        end
        object edtMascaraInscEst: TDBEdit
          Left = 512
          Top = 120
          Width = 129
          Height = 21
          DataField = 'MASCARA_INSCRICAO_ESTADUAL'
          DataSource = dmMSource.dtsEstados
          MaxLength = 5
          TabOrder = 10
          OnEnter = edtSiglaEnter
          OnExit = edtAliqICMSIntExit
          OnKeyDown = edtConteudoPesquisaKeyDown
          OnKeyPress = edtAliqICMSIntKeyPress
        end
        object chkUtilizaAtivEstadual: TDBCheckBox
          Left = 371
          Top = 145
          Width = 155
          Height = 17
          Alignment = taLeftJustify
          Caption = 'Utiliza Ativ. Est.:'
          DataField = 'UTILIZA_ATIVIDADE_ESTADUAL'
          DataSource = dmMSource.dtsEstados
          TabOrder = 11
          ValueChecked = '1'
          ValueUnchecked = '0'
        end
        object edtMascaraAtivEst: TDBEdit
          Left = 512
          Top = 166
          Width = 129
          Height = 21
          DataField = 'MASCARA_INSCRICAO_ESTADUAL'
          DataSource = dmMSource.dtsEstados
          MaxLength = 5
          TabOrder = 12
          OnEnter = edtSiglaEnter
          OnExit = edtAliqICMSIntExit
          OnKeyDown = edtConteudoPesquisaKeyDown
          OnKeyPress = edtAliqICMSIntKeyPress
        end
        object chkConvenioSimplesFederal: TDBCheckBox
          Left = 371
          Top = 190
          Width = 155
          Height = 17
          Alignment = taLeftJustify
          Caption = 'Conveniado Simples Federal'
          DataField = 'SIMPLES_FEDERAL'
          DataSource = dmMSource.dtsEstados
          TabOrder = 13
          ValueChecked = '1'
          ValueUnchecked = '0'
        end
        object edtAliquota_Simples_Federal: TDBEdit
          Left = 512
          Top = 211
          Width = 62
          Height = 21
          DataField = 'ALIQUOTA_SIMPLES_FEDERAL'
          DataSource = dmMSource.dtsEstados
          MaxLength = 2
          TabOrder = 16
          OnEnter = edtSiglaEnter
          OnExit = edtCodigoIBGEExit
          OnKeyDown = edtConteudoPesquisaKeyDown
          OnKeyPress = edtConteudoPesquisaKeyPress
        end
      end
      object grpCidades: TGroupBox
        Left = 3
        Top = 247
        Width = 653
        Height = 117
        Caption = 'CIDADES'
        TabOrder = 1
        object grdCidades: TDBGrid
          Left = 2
          Top = 15
          Width = 649
          Height = 100
          Align = alClient
          BorderStyle = bsNone
          DataSource = dmMSource.dtsCidades
          DrawingStyle = gdsClassic
          Options = [dgTitles, dgIndicator, dgColLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit, dgTitleHotTrack]
          ReadOnly = True
          TabOrder = 0
          TitleFont.Charset = DEFAULT_CHARSET
          TitleFont.Color = clWindowText
          TitleFont.Height = -11
          TitleFont.Name = 'Tahoma'
          TitleFont.Style = []
          OnDrawColumnCell = grdCidadesDrawColumnCell
          Columns = <
            item
              Expanded = False
              FieldName = 'NOME'
              Title.Alignment = taCenter
              Width = 402
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'CODIGO_MUNICIPIO'
              Title.Alignment = taCenter
              Title.Caption = 'C'#211'DIGO. IBGE'
              Width = 83
              Visible = True
            end>
        end
      end
    end
    object TabSheet1: TTabSheet
      Caption = 'FECP- Fundo de Erradica'#231#227'o e Combate a Pobreza'
      ImageIndex = 2
      ExplicitHeight = 296
      object grpFECP: TGroupBox
        Left = 0
        Top = 0
        Width = 659
        Height = 366
        Align = alClient
        Enabled = False
        TabOrder = 0
        ExplicitHeight = 296
        object imgVincularFECP_NCM: TImage
          Left = 16
          Top = 15
          Width = 43
          Height = 39
          Cursor = crHandPoint
          Hint = 'Vincular NCM/FECP'
          ParentShowHint = False
          Picture.Data = {
            
            8176DB2F8F5A3BCF6F01AB670000000049454E44AE426082}
          ShowHint = True
          Stretch = True
          OnClick = imgVincularFECP_NCMClick
        end
        object grdFECP: TDBGrid
          Left = 2
          Top = 133
          Width = 655
          Height = 231
          Hint = '<DELETE> Remove NCM vinculado'
          Align = alBottom
          DataSource = dmMSource.dtsFCP_Estados
          DrawingStyle = gdsClassic
          Options = [dgTitles, dgIndicator, dgColLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit, dgTitleClick, dgTitleHotTrack]
          ParentShowHint = False
          ShowHint = True
          TabOrder = 0
          TitleFont.Charset = DEFAULT_CHARSET
          TitleFont.Color = clWindowText
          TitleFont.Height = -11
          TitleFont.Name = 'Tahoma'
          TitleFont.Style = []
          OnDrawColumnCell = grdFECPDrawColumnCell
          OnKeyDown = grdFECPKeyDown
          Columns = <
            item
              Expanded = False
              FieldName = 'SIGLA'
              Title.Alignment = taCenter
              Title.Caption = 'ESTADO'
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'NCM'
              Title.Alignment = taCenter
              Width = 72
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'FCP'
              Title.Alignment = taCenter
              Title.Caption = '% FCP'
              Width = 49
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'ALIQUOTA_INTERNA_DESTINO'
              Title.Alignment = taCenter
              Title.Caption = '% Aliq.Intra'
              Width = 67
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'S_EXCECAO'
              Title.Alignment = taCenter
              Title.Caption = 'EXCE'#199#195'O'
              Width = 56
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'NOME_NCM'
              Title.Alignment = taCenter
              Title.Caption = 'Descri'#231#227'o'
              Width = 331
              Visible = True
            end>
        end
        object grpPesquisaNCM_FCP: TGroupBox
          Left = 293
          Top = 6
          Width = 364
          Height = 48
          Caption = 'Pesquisar NCM'
          TabOrder = 1
          object imgPesquisarNCM_FCP: TImage
            Left = 331
            Top = 19
            Width = 27
            Height = 22
            Cursor = crHandPoint
            Hint = 'pesquisar'
            ParentShowHint = False
            Picture.Data = {
              
              00000049454E44AE426082}
            ShowHint = True
            Stretch = True
            OnClick = edtPesquisaNCM_FCPExit
          end
          object edtPesquisaNCM_FCP: TEdit
            Left = 8
            Top = 20
            Width = 318
            Height = 21
            CharCase = ecUpperCase
            TabOrder = 0
            TextHint = 'Digite aqui para pesquisar, e pressione <ENTER>'
            OnEnter = edtPesquisaNCM_FCPEnter
            OnExit = edtPesquisaNCM_FCPExit
            OnKeyDown = edtConteudoPesquisaKeyDown
            OnKeyPress = edtConteudoPesquisaKeyPress
          end
        end
      end
    end
  end
  object grpPesquisa: TGroupBox
    Left = 8
    Top = 11
    Width = 667
    Height = 70
    Caption = 'Pesquisa'
    TabOrder = 2
    object rdpOpcoesPesquisa: TRadioGroup
      Left = 8
      Top = 18
      Width = 289
      Height = 48
      Cursor = crHandPoint
      Caption = 'Op'#231#245'es'
      Columns = 2
      ItemIndex = 0
      Items.Strings = (
        'Sigla'
        'Nome')
      TabOrder = 0
      OnClick = rdpOpcoesPesquisaClick
    end
    object grpConteudoPesquisa: TGroupBox
      Left = 299
      Top = 19
      Width = 364
      Height = 48
      Caption = 'Conte'#250'do'
      TabOrder = 1
      object impPesquisar: TImage
        Left = 331
        Top = 19
        Width = 27
        Height = 22
        Cursor = crHandPoint
        Hint = 'pesquisar'
        ParentShowHint = False
        Picture.Data = {
          
          00000049454E44AE426082}
        ShowHint = True
        Stretch = True
        OnClick = edtConteudoPesquisaExit
      end
      object edtConteudoPesquisa: TEdit
        Left = 8
        Top = 20
        Width = 318
        Height = 21
        CharCase = ecUpperCase
        TabOrder = 0
        TextHint = 'Digite aqui para pesquisar, e pressione <ENTER>'
        OnEnter = edtConteudoPesquisaEnter
        OnExit = edtConteudoPesquisaExit
        OnKeyDown = edtConteudoPesquisaKeyDown
        OnKeyPress = edtConteudoPesquisaKeyPress
      end
    end
  end
  object ActionList1: TActionList
    Left = 516
    Top = 84
    object WindowClose1: TWindowClose
      Category = 'Window'
      Caption = 'C&lose'
      Enabled = False
      Hint = 'Close'
      OnExecute = WindowClose1Execute
    end
    object actIncluir: TAction
      Category = 'Window'
      Caption = 'actIncluir'
      ShortCut = 113
      OnExecute = actIncluirExecute
    end
    object actDesfazer: TAction
      Category = 'Window'
      Caption = 'actDesfazer'
      ShortCut = 114
      OnExecute = actDesfazerExecute
    end
    object actSalvar: TAction
      Category = 'Window'
      Caption = 'actSalvar'
      ShortCut = 115
      OnExecute = actSalvarExecute
    end
    object actEditar: TAction
      Category = 'Window'
      Caption = 'actEditar'
      ShortCut = 117
      OnExecute = actEditarExecute
    end
    object actExcluir: TAction
      Category = 'Window'
      Caption = 'actExcluir'
      OnExecute = actExcluirExecute
    end
    object actExcluirNCM_FCP: TAction
      Category = 'Window'
      Caption = 'actExcluirNCM_FCP'
      OnExecute = actExcluirNCM_FCPExecute
    end
  end
end
