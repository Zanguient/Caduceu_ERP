...
uses
  ...
  Dialogs, ComCtrls,  Buttons, StdCtrls, DBCtrls, pngimage, ExtCtrls,
...
//    extRControlePerda: TEXLReport;
    ...
    chkNCMObsoletos: TCheckBox;
    tbsRelPedidos: TTabSheet;
    grpPeriodoListagemPedidosVenda: TGroupBox;
    lblDataIniPedidoVenda: TLabel;
    lblDataFinalPedidoVenda: TLabel;
    dtpDataIniPedidoVenda: TDateTimePicker;
    dtpDataFinalPedidoVenda: TDateTimePicker;
    rdgTipoPedidoVenda: TRadioGroup;
    rdgStatusPedidoVenda: TRadioGroup;
    rdgTipoFpagtoPedidoVenda: TRadioGroup;
    grpClientePedidovenda: TGroupBox;
    grpVenededorPedidoVenda: TGroupBox;
    cboClientePedidoVenda: TDBLookupComboBox;
    cboVendedorPedidoVenda: TDBLookupComboBox;
    imgClientePedVenda: TImage;
    imgVendedorPedVenda: TImage;
    tbsListagemNFEntrada: TTabSheet;
    grpFornecedorListagemNFEntrada: TGroupBox;
    lblFornecedorListagemNFEntrada: TLabel;
    edtFornecedorListagemNFEntrada: TEdit;
    Label12: TLabel;
    edtNomeRSFornecedorListNFEntrada: TEdit;
    imgPesqFornecedorRSocial: TImage;
    grpCompradorListNFEntrada: TGroupBox;
    cboCompradorListNFEntrada: TDBLookupComboBox;
    imgCompradorListNFEntrada: TImage;
    grpOpcaoDatasListNFEntrada: TGroupBox;
    rdglDataListNFEntrada: TRadioGroup;
    grpDatasListNFEntrada: TGroupBox;
    lblDataIniListNFEntrada: TLabel;
    lblDataFinListNFEntrada: TLabel;
    dtpDataIniListNFEntrada: TDateTimePicker;
    dtpDataFinListNFEntrada: TDateTimePicker;
    rdgOrdenecaoLisNFEntrada: TRadioGroup;
    rdgFinalidadeNF: TRadioGroup;
    imgExportarExcel: TImage;
    tbsListagemNFSaida: TTabSheet;
    grpDestinatarioNFSaida: TGroupBox;
    lblDestinatarioNFSaida: TLabel;
    lblRSocialDestinatario: TLabel;
    imgPesquisaClienteNFS: TImage;
    edtCNPJClienteNFS: TEdit;
    edtRSocialDestinatarioNFS: TEdit;
    grpEmissaoNFS: TGroupBox;
    rdgStatusNFS: TRadioGroup;
    lblDataIniNFS: TLabel;
    dtpDataIniNFS: TDateTimePicker;
    lblDataFinNFS: TLabel;
    dtpDataFinNFS: TDateTimePicker;
    ...
    procedure tbsRelPedidosShow(Sender: TObject);
    procedure imgClientePedVendaClick(Sender: TObject);
    procedure imgVendedorPedVendaClick(Sender: TObject);
    procedure tbsListagemNFEntradaShow(Sender: TObject);
    procedure imgPesqFornecedorRSocialClick(Sender: TObject);
    procedure imgCompradorListNFEntradaClick(Sender: TObject);
    procedure edtFornecedorListagemNFEntradaExit(Sender: TObject);
    procedure edtFornecedorListagemNFEntradaKeyPress(Sender: TObject;
      var Key: Char);
    procedure imgExportarExcelClick(Sender: TObject);
    procedure tbsListagemNFSaidaShow(Sender: TObject);
    procedure edtCNPJClienteNFSExit(Sender: TObject);
  ...
  private
    ...
    procedure MontarSQL_ListagemPedvenda;
    ...
    procedure AbrirTabelaClientes;
    procedure AbrirTabelaFuncionarios;
    procedure MontarSQL_ListagemNFEntrada;
    procedure MontarSQL_ListagemNFSaida(pOpcao:smallint);
  public
    { Public declarations }
  end;
...
uses dataDBEXMaster, dataMProvider, dataMRelatorios, dataMSource,
  ...
  modPesquisaSetorBalanca, uFuncoes_BD;
...
procedure TfrmRelatorios.AbrirTabelaClientes;
begin
  ...
  dmDBEXMaster.fdqClientes.SQL.Add('order by clientes.razaosocial');
...
procedure TfrmRelatorios.AbrirTabelaFuncionarios;
begin
  ...
  dmDBEXMaster.fdqFuncionarios.SQL.Add('select * from funcionario order by nome');
...
begin
  ...
  begin
    ...
    dmRelatorios.fdqRegInventarioP7.SQL.Add('where inventario.status = 1');
...
procedure TfrmRelatorios.edtCNPJClienteNFSExit(Sender: TObject);
begin

  MudarCorEdit(Sender);
  LimparMsgErro;

  if Length(Trim(edtCNPJClienteNFS.Text)) > 0 then
  begin
    ...
    if Length(trim(edtCNPJClienteNFS.Text)) <= 10 then
      dmDBEXMaster.fdqClientes.SQL.Add('WHERE CLIENTES.CLIENTE = ' + edtCNPJClienteNFS.Text)
    else if Length(trim(edtCNPJClienteNFS.Text)) > 10 then
      dmDBEXMaster.fdqClientes.SQL.Add('WHERE CLIENTES.CNPJ = ' + QuotedStr(Trim(edtCNPJClienteNFS.Text)));
    ...
    begin
      ...
      lblMsg.Caption := dmDBEXMaster.sNomeUsuario + MSG_PESQUISA;
      Application.ProcessMessages;
      edtCNPJClienteNFS.SetFocus;
      edtCNPJClienteNFS.SelectAll;
    ...
    begin
      ...
      LimparMsgErro;
      edtCNPJClienteNFS.Text          := IntToStr(dmMProvider.cdsClientesCLIENTE.Value);
      edtRSocialDestinatarioNFS.Text  := dmMProvider.cdsClientesRAZAOSOCIAL.Value;

    end;

  end
  else
    edtRSocialDestinatarioNFS.Clear;
...
begin
  ...
  //codigo provisorio ate migração total para dmcdsmaster;
...
procedure TfrmRelatorios.edtFornecedorListagemNFEntradaExit(Sender: TObject);
begin

  MudarCorEdit(Sender);
  LimparMsgErro;

  if Length(Trim(edtFornecedorListagemNFEntrada.Text)) > 0 then
  begin
    ...
    if Length(trim(edtFornecedorListagemNFEntrada.Text)) <= 10 then
      dmDBEXMaster.fdqFornecedor.SQL.Add('WHERE FORNECEDOR.FORNECEDOR = ' + edtFornecedorListagemNFEntrada.Text)
    else if Length(trim(edtFornecedorListagemNFEntrada.Text)) > 10 then
      dmDBEXMaster.fdqFornecedor.SQL.Add('WHERE FORNECEDOR.CNPJ = ' + QuotedStr(Trim(edtFornecedorListagemNFEntrada.Text)));
    ...
    begin
      ...
      lblMsg.Caption := dmDBEXMaster.sNomeUsuario + MSG_PESQUISA;
      Application.ProcessMessages;
      edtFornecedorListagemNFEntrada.SetFocus;
      edtFornecedorListagemNFEntrada.SelectAll;
    ...
    begin
      ...
      LimparMsgErro;
      edtFornecedorListagemNFEntrada.Text   := IntToStr(dmMProvider.cdsFornecedorFORNECEDOR.Value);
      edtNomeRSFornecedorListNFEntrada.Text := dmMProvider.cdsFornecedorRAZAOSOCIAL.Value;
...
procedure TfrmRelatorios.edtFornecedorListagemNFEntradaKeyPress(Sender: TObject;
  var Key: Char);
begin
...
begin
  ...
  begin
    ...
    dmDBEXMaster.fdqPesqProdutos.SQL.Add('SELECT PRO.*, PF.* FROM PRODUTO PRO');
    dmDBEXMaster.fdqPesqProdutos.SQL.Add('JOIN PRODUTOFILIAL PF');
    dmDBEXMaster.fdqPesqProdutos.SQL.Add('ON(PF.PRODUTO = PRO.PRODUTO)');
    dmDBEXMaster.fdqPesqProdutos.SQL.Add('WHERE PRO.PRODUTO = ' + edtProdutoFin.Text);
    ...
    begin
      ...
      edtNomeProdutoFin.Text  := dmMProvider.cdsPesqProdutosDESCRICAO.Value;
      LimparMsgErro;
...
begin
  ...
  begin
    ...
    dmDBEXMaster.fdqPesqProdutos.SQL.Add('JOIN PRODUTOFILIAL PF');
    dmDBEXMaster.fdqPesqProdutos.SQL.Add('ON(PF.PRODUTO = PRO.PRODUTO)');
    dmDBEXMaster.fdqPesqProdutos.SQL.Add('WHERE PRO.PRODUTO = ' + edtProdutoIni.Text);
    ...
    begin
      ...
      edtNomeProdutoIni.Text  := dmMProvider.cdsPesqProdutosDESCRICAO.Value;
      LimparMsgErro;
...
procedure TfrmRelatorios.imgPesqFornecedorRSocialClick(Sender: TObject);
begin
...
procedure TfrmRelatorios.imgVendedorPedVendaClick(Sender: TObject);
begin

  cboVendedorPedidoVenda.KeyValue := -1;

end;

procedure TfrmRelatorios.imgClientePedVendaClick(Sender: TObject);
begin

  cboClientePedidoVenda.KeyValue := -1;

end;

procedure TfrmRelatorios.imgCompradorListNFEntradaClick(Sender: TObject);
begin

  cboCompradorListNFEntrada.KeyValue := -1;

end;

procedure TfrmRelatorios.imgExportarExcelClick(Sender: TObject);
begin

  case pgcFiltro.Tag of
         ord(rlNFEntrada):begin

                            MontarSQL_ListagemNFEntrada;

                            LimparMsgErro;

                            if ExportarExcel(dmRelatorios.cdsListNFEntrada,'Relatorios_NFEntrada.CSV', False,0) then
                            begin

                              dmRelatorios.cdsListNFEntrada.Close;

                              lblMsg.Caption := dmDBEXMaster.sNomeUsuario + ', arquivo: ' + ExtractFilePath(ParamStr(0)) + 'CSV\Relatorios_NFEntrada.CSV'
                                                + ' gerado com sucesso';
                              Application.ProcessMessages;
           ...
           ord(rlNFSaida):begin
                            ...
                            MontarSQL_ListagemNFSaida(0);

                            LimparMsgErro;

                            if ExportarExcel(dmRelatorios.cdsListNFsaida,'Relatorios_NFSaida.CSV', False, 1) then
                            begin

                              dmRelatorios.cdsListNFsaida.Close;

                              lblMsg.Caption := dmDBEXMaster.sNomeUsuario + ', arquivo: ' + ExtractFilePath(ParamStr(0)) + 'CSV\Relatorios_NFSaida.CSV'
                                                + ' gerado com sucesso';
                              Application.ProcessMessages;

                              //totaliza cfop por nota
                              MontarSQL_ListagemNFSaida(1);

                              if ExportarExcel(dmRelatorios.cdsTotCFOP_NFS,'Relatorios_NFSaida.CSV', true, 2) then
                              begin

                                dmRelatorios.cdsTotCFOP_NFS.Close;
                                ...
                                //totaliza cfop
                                MontarSQL_ListagemNFSaida(2);

                                if ExportarExcel(dmRelatorios.cdsTotGeralCFOP_NFS,'Relatorios_NFSaida.CSV', true, 3) then
                                begin

                                  dmRelatorios.cdsTotGeralCFOP_NFS.Close;
...
begin
  ...
  lblMsg.Caption := 'Aguarde....gerando relatório';
  Application.ProcessMessages;
  ...
  case pgcFiltro.Tag of
         ord(rlNFEntrada):begin
                            ...
                            dmRelatorios.rvdGenesis.DataSet := dmRelatorios.cdsListNFEntrada;
                            dmRelatorios.rvsGenesisAC.SystemPreview.ZoomFactor  := 126.4;
                            ...
                            if FileExists(ExtractFilePath(Application.ExeName)+'Rav\Relatorios_NotasFiscais.rav') then
                            begin

                              dmRelatorios.rvpGenesisAC.ProjectFile := ExtractFilePath(Application.ExeName)+'Rav\Relatorios_NotasFiscais.rav';
                              dmRelatorios.rvpGenesisAC.SelectReport('rptListagemNFEntrada',true);
                              dmRelatorios.rvpGenesisAC.SetParam('pID_SISTEMA',ID_SISTEMA);
                              dmRelatorios.rvpGenesisAC.SetParam('pPeriodo','Período entre: ' + FormatDateTime('dd/mm/yyyy', dtpDataIniNFS.Date) + ' a ' + FormatDateTime('dd/mm/yyyy', dtpDataFinNFS.Date));
                              ...
                              + ExtractFilePath(Application.ExeName)+'Rav\Relatorios_NotasFiscais.rav'), 'Atenção!',mb_IconWarning + mb_Ok);

                          end;
           ord(rlNFSaida):begin

                            dmRelatorios.rvdGenesis.DataSet := dmRelatorios.cdsListNFsaida;
                            dmRelatorios.rvsGenesisAC.SystemPreview.ZoomFactor  := 126.4;
                            ...
                            begin
                              ...
                              dmRelatorios.rvpGenesisAC.ProjectFile := ExtractFilePath(Application.ExeName)+'Rav\Relatorios_NotasFiscais.rav';
                              dmRelatorios.rvpGenesisAC.SelectReport('rptListagemNFSaida',true);
                              dmRelatorios.rvpGenesisAC.SetParam('pID_SISTEMA',ID_SISTEMA);
                              dmRelatorios.rvpGenesisAC.SetParam('pID_RELATORIO','rptListagemNFSaida');
                              dmRelatorios.rvpGenesisAC.SetParam('pPeriodo','Período entre: ' + FormatDateTime('dd/mm/yyyy', dtpDataIniListNFEntrada.Date) + ' a ' + FormatDateTime('dd/mm/yyyy', dtpDataFinListNFEntrada.Date));
                              dmRelatorios.rvpGenesisAC.SetParam('pNOME_LOJA',sFilial);
                              dmRelatorios.rvpGenesisAC.SetParam('pLOGO_MARCA',ExtractFilePath(Application.ExeName) + 'Logomarca\LogoMarca.bmp');
                              dmRelatorios.rvpGenesisAC.Execute;
                              dmRelatorios.rvpGenesisAC.Close;
    ...
    ord(rlListagemPedido):begin

                            dmRelatorios.rvsGenesisAC.SystemPreview.ZoomFactor  := 126.4;

                            MontarSQL_ListagemPedvenda;
                            ...
                            begin
                              ...
                              dmRelatorios.rvpGenesisAC.SelectReport('rptListagemPedidoVenda',true);
                              dmRelatorios.rvpGenesisAC.SetParam('pNOME_LOJA',sFilial);
                              dmRelatorios.rvpGenesisAC.SetParam('pLOGO_MARCA',ExtractFilePath(Application.ExeName) + 'Logomarca\LogoMarca.bmp');
                              dmRelatorios.rvpGenesisAC.Execute;
                              dmRelatorios.rvpGenesisAC.Close;
  ...
  case tbsPerdas.Tag of
    ord(rlControlePerda):begin
                          ...
                          begin
                            ...
                            dmRelatorios.rvpGenesisAC.SetParam('pNOME_LOJA',sFilial);
                            dmRelatorios.rvpGenesisAC.SetParam('pLOGO_MARCA',ExtractFilePath(Application.ExeName) + 'Logomarca\LogoMarca.bmp');
                            dmRelatorios.rvpGenesisAC.SetParam('pBairro','Período entre ' + FormatDateTime('dd/mm/yyyy', dtpDataIniPerdas.Date) + ' e ' + FormatDateTime('dd/mm/yyyy', dtpDataFinPerdas.Date));
                            dmRelatorios.rvpGenesisAC.Execute;
                            dmRelatorios.rvpGenesisAC.Close;
                            LimparCampos;
  ...
  case tbsRelProdutos.Tag of
    ord(rlTabGrupoCstICMSTrib):begin

                                dmRelatorios.rvdGenesis.DataSet := dmMProvider.cdsProdutos;

                                MontarSQL_Produto(ord(rlTabGrupoCstICMSTrib));
                                ...
                                begin
                                  ...
                                  dmRelatorios.rvpGenesisAC.SelectReport('rptGRUPO_CST_Trib',true);
                                  ...
                                  Application.MessageBox(PChar(dmDBEXMaster.sNomeUsuario + ', ' + MSG_ARQ_NEXISTE + #13
                                  + ExtractFilePath(Application.ExeName)+'Rav\Relatorios_Produtos.rav'), 'Atenção!',mb_IconWarning + mb_Ok);

                               end;

    ord(rlTabCstICMSTrib):begin

                            dmRelatorios.rvdGenesis.DataSet := dmMProvider.cdsProdutos;

                            MontarSQL_Produto(ord(rlTabCstICMSTrib));
                            ...
                            begin
                              ...
                              dmRelatorios.rvpGenesisAC.ProjectFile := ExtractFilePath(Application.ExeName)+'Rav\Relatorios_Produtos.rav';
                              dmRelatorios.rvpGenesisAC.SelectReport('rptProdCST_ICMS_Trib',true);
                              dmRelatorios.rvpGenesisAC.SetParam('P_NOME_LOJA',sFilial);
                              dmRelatorios.rvpGenesisAC.SetParam('pID_SISTEMA',ID_SISTEMA);
                              dmRelatorios.rvpGenesisAC.SetParam('pLOGO_MARCA',ExtractFilePath(Application.ExeName) + 'Logomarca\LogoMarca.bmp');
                              dmRelatorios.rvpGenesisAC.Execute;
    ...
    ord(rlTabProdutoPromocao):begin
                                ...
                                begin
                                  ...
                                  dmRelatorios.rvpGenesisAC.SetParam('P_TITULO_RELATORIO','LISTAGEM DE PRODUTOS E PROMOÇÃO');
    ...
    ord(rlApoioPromocao):begin
                          ...
                          begin
                            ...
                            dmRelatorios.rvpGenesisAC.SetParam('P_NOME_LOJA',sFilial);
                            dmRelatorios.rvpGenesisAC.SetParam('P_LOGO_MARCA',ExtractFilePath(Application.ExeName) + 'Logomarca\LogoMarca.bmp');
                            dmRelatorios.rvpGenesisAC.SetParam('P_TITULO_RELATORIO','LISTAGEM DE PROMOÇÕES ATIVAS');
                            dmRelatorios.rvpGenesisAC.Open;
                            dmRelatorios.rvpGenesisAC.ExecuteReport('rptTab_Promocao');
                            dmRelatorios.rvpGenesisAC.Close;
                            LimparCampos;
    ...
    ord(rlApoioInvetario):begin
                            ...
                            begin
                              ...
                              Application.MessageBox(PChar(dmDBEXMaster.sNomeUsuario + ', ' + MSG_ARQ_NEXISTE + #13
                                + ExtractFilePath(Application.ExeName)+'Rav\RAInventario.rav'), 'Atenção!',mb_IconWarning + mb_Ok);
  ...
  case tbsRegistroInventario.Tag of
      ...
      ord(rlRegistroInventaroP7):begin
                                    ...
                                    begin
                                      ...
                                      dmRelatorios.rvpGenesisAC.SetParam('pMUNICIPIO',dmMProvider.cdsFilialNOMECIDADE.Value);
...
//                                  dmRelatorios.rvdGenesis.DataSet := dmRelatorios.cdsVendaPisCofins;
                                  ...
                                  begin
                                    ...
                                    dmRelatorios.rvpGenesisAC.SetParam('pNOME_LOJA',sFilial);
                                    dmRelatorios.rvpGenesisAC.SetParam('pLOGO_MARCA',ExtractFilePath(Application.ExeName) + 'Logomarca\LogoMarca.bmp');
                                    dmRelatorios.rvpGenesisAC.SetParam('pPERIODO','Período ' + FormatDateTime('dd/mm/yyyy', dtpDataInicialPISCOFINS.DateTime)
                                    ...
                                    Application.MessageBox(PChar(dmDBEXMaster.sNomeUsuario + ', ' + MSG_ARQ_NEXISTE + #13
                                    + ExtractFilePath(Application.ExeName)+'Rav\RelVendas.rav'), 'Atenção!',mb_IconWarning + mb_Ok);
                                  end;
    ord(rlVendaProdutoCFOP):begin
                                  ...
                                  begin
                                    ...
                                    Application.MessageBox(PChar(dmDBEXMaster.sNomeUsuario + ', ' + MSG_ARQ_NEXISTE + #13
                                    + ExtractFilePath(Application.ExeName)+'Rav\Relatatorio_Movimento.rav'), 'Atenção!',mb_IconWarning + mb_Ok);
                                  end;
...
//                                    dmRelatorios.rvpGenesisAC.SetParam('pLOGO_MARCA',ifArqIni.ReadString('ConfDANFE', 'LogoMarca', ''));
                                    dmRelatorios.rvpGenesisAC.SetParam('pPERIODO','Período ' + FormatDateTime('dd/mm/yyyy', dtpDataInicialRanking.DateTime)
                                    ...
                                    begin
                                      ...
                                      lblMsg.Caption := dmDBEXMaster.sNomeUsuario + MSG_GERANDO_PLANILAH_EXCEL;
                                      Application.ProcessMessages;
...
begin
  ...
  begin
    ...
    if lstItens.Items[lstItens.ItemIndex].Caption  = 'Tabela de preços' then
    begin
    ...
    else if lstItens.Items[lstItens.ItemIndex].Caption  = 'Tabela de Produtos em promoção' then
    begin
    ...
    else if lstItens.Items[lstItens.ItemIndex].Caption  = 'Relatório de venda de produto por CFOP' then
    begin
    ...
    else if lstItens.Items[lstItens.ItemIndex].Caption  = 'Relatório de compra de produto por CFOP' then
    begin
    ...
    else if lstItens.Items[lstItens.ItemIndex].Caption  = 'Tabela de Apoio a Promoção' then
    begin
    ...
    else if lstItens.Items[lstItens.ItemIndex].Caption  = 'Relatório de Venda por Grupo' then
    begin
    ...
    else if lstItens.Items[lstItens.ItemIndex].Caption  = 'Venda por Tributação' then
    begin
    ...
    else if lstItens.Items[lstItens.ItemIndex].Caption  = 'Tabela de Apoio ao Inventário' then
    begin
    ...
    else if lstItens.Items[lstItens.ItemIndex].Caption  = 'Relatório de Estados' then
    begin
    ...
    else if lstItens.Items[lstItens.ItemIndex].Caption  = 'Relatório de Cidades' then
    begin
    ...
    else if lstItens.Items[lstItens.ItemIndex].Caption  = 'Notas Fiscais de Entrada' then
    begin

      LimparMsgErro;
      tbsListagemNFEntrada.TabVisible := True;
      pgcFiltro.Tag                   := ord(rlNFEntrada);
      imgExportarExcel.Visible        := True;

    end
    else if lstItens.Items[lstItens.ItemIndex].Caption  = 'Notas Fiscais de Saída' then
    begin

      LimparMsgErro;
      tbsListagemNFSaida.TabVisible   := True;
      pgcFiltro.Tag                   := ord(rlNFSaida);
      imgExportarExcel.Visible        := True;

    end
    else if lstItens.Items[lstItens.ItemIndex].Caption  = 'Relatório de Bancos' then
    begin
    ...
    else if lstItens.Items[lstItens.ItemIndex].Caption  = 'Relatório de Administradoras de cartões' then
    begin
    ...
    else if lstItens.Items[lstItens.ItemIndex].Caption  = 'Registro de Inventário P7' then
    begin
    ...
    else if lstItens.Items[lstItens.ItemIndex].Caption  = 'Livro Registro de Inventário P7' then
    begin
    ...
    else if lstItens.Items[lstItens.ItemIndex].Caption  = 'Relatório de apoio EFD-Contribuições' then
    begin
    ...
    else if lstItens.Items[lstItens.ItemIndex].Caption  = 'Tabela Grupo x CST ICMS x Tributação' then
    begin

      LimparMsgErro;
      tbsRelProdutos.TabVisible  := True;
      tbsRelProdutos.Tag         := ord(rlTabGrupoCstICMSTrib);

    end
    else if lstItens.Items[lstItens.ItemIndex].Caption  = 'Tabela CST ICMS x Tributação' then
    begin

      LimparMsgErro;
      tbsRelProdutos.TabVisible  := True;
      tbsRelProdutos.Tag         := ord(rlTabCstICMSTrib);

    end
    else if lstItens.Items[lstItens.ItemIndex].Caption  = 'Listagem de pedidos' then
    begin

      if not dmMProvider.cdsFuncionarios.Active then
        AbrirTabelaFuncionarios;

      if not dmMProvider.cdsClientes.Active then
        AbrirTabelaClientes;

      LimparMsgErro;
      tbsRelPedidos.TabVisible  := True;
      tbsRelPedidos.Tag         := ord(rlListagemPedido);
      pgcFiltro.Tag             := tbsRelPedidos.Tag;
...
procedure TfrmRelatorios.MontarSQL_ListagemPedvenda;
var
  sWhere:string;
begin
  ...
  dmRelatorios.cdsListagemPedVenda.Close;
  dmRelatorios.cdsListagemPedVenda.ProviderName := 'dspListagemPedVenda';

  dmRelatorios.fdqListagemPedVenda.SQL.Clear;
  dmRelatorios.fdqListagemPedVenda.SQL.Add('select pvz.pedido, pvz.emissao, pvz.situacao, pvz.vendedor, pvz.cliente, pvz.valor_do_pedido,');
  dmRelatorios.fdqListagemPedVenda.SQL.Add('cli.razaosocial, func.nome, fpg.tipo from pedido_venda_zeus pvz');
  dmRelatorios.fdqListagemPedVenda.SQL.Add('left join clientes cli');
  dmRelatorios.fdqListagemPedVenda.SQL.Add('on(cli.cliente = pvz.cliente)');
  dmRelatorios.fdqListagemPedVenda.SQL.Add('join funcionario func');
  dmRelatorios.fdqListagemPedVenda.SQL.Add('on(func.funcionario = pvz.vendedor)');
  dmRelatorios.fdqListagemPedVenda.SQL.Add('join finalizadora_pedido_zeus fin');
  dmRelatorios.fdqListagemPedVenda.SQL.Add('on(fin.pedido = pvz.pedido)');
  dmRelatorios.fdqListagemPedVenda.SQL.Add('join formapagamento fpg');
  dmRelatorios.fdqListagemPedVenda.SQL.Add('on(fpg.formapagamento = fin.forma_pagamento)');

  sWhere := 'where pvz.emissao between ' + QuotedStr(FormatDateTime('mm/dd/yyyy', dtpDataIniPedidoVenda.Date))
            + ' and ' + QuotedStr(FormatDateTime('mm/dd/yyyy', dtpDataFinalPedidoVenda.Date)) + ' and pvz.filial = ' + IntToStr(dmDBEXMaster.iIdFilial);

  case rdgTipoPedidoVenda.ItemIndex of
    0,1:sWhere := sWhere + ' and pvz.pedido_orcamento = ' + IntToStr(rdgTipoPedidoVenda.ItemIndex);
  end;

  case rdgStatusPedidoVenda.ItemIndex of
    0,1,2:sWhere := sWhere + ' and pvz.situacao = ' + IntToStr(rdgStatusPedidoVenda.ItemIndex);
  end;

  if rdgTipoFpagtoPedidoVenda.ItemIndex <= 10 then
    sWhere := sWhere + ' and fpg.tipo = ' + IntToStr(rdgTipoFpagtoPedidoVenda.ItemIndex);

  if Length(trim(cboClientePedidoVenda.Text)) > 0 then
    sWhere := sWhere + ' and pvz.cliente = ' + IntToStr(cboClientePedidoVenda.KeyValue);

  if Length(trim(cboVendedorPedidoVenda.Text)) > 0 then
    sWhere := sWhere + ' and pvz.vendedor = ' + IntToStr(cboVendedorPedidoVenda.KeyValue);

  dmRelatorios.fdqListagemPedVenda.SQL.Add(sWhere);

  dmRelatorios.fdqListagemPedVenda.SQL.Add('order by fpg.tipo, pvz.pedido, cli.razaosocial');

  dmRelatorios.cdsListagemPedVenda.Open;
  dmRelatorios.cdsListagemPedVenda.ProviderName := '';
...
begin
  ...
  case pRelatorio of
   ...
   10:begin//Registro de inventario P7
        ...
        dmRelatorios.fdqRegInventarioP7.SQL.Clear;
        dmRelatorios.fdqRegInventarioP7.SQL.Add('SELECT INVENTARIO.INVENTARIO, INVENTARIO.data, INVENTARIO.anodereferencia, INVENTARIO.tipocustoinventario, INVENTARIO.valorliquidodoinventario,');
        dmRelatorios.fdqRegInventarioP7.SQL.Add('INVENTARIO.numero_ordem, iteminventario.quantidade, iteminventario.precodecusto,');
        dmRelatorios.fdqRegInventarioP7.SQL.Add('(iteminventario.precodecusto * iteminventario.quantidade) subtotal,');
        dmRelatorios.fdqRegInventarioP7.SQL.Add('produto.descricao as descricao_produto, produto.unidade, produto.tributacao,');
        dmRelatorios.fdqRegInventarioP7.SQL.Add('aliquotas.nome as descricao_tributacao, iteminventario.produto from inventario');
   ...
   14:begin
        ...
        if chkNCMObsoletos.Checked then
          dmRelatorios.fdqApoioEFDContr.SQL.Add('AND PRO.NCM NOT IN (SELECT COD_NCM FROM NBM_NCM)');
   ...
   19:begin //rlTabGrupoCstICMSTrib
        ...
        dmDBEXMaster.fdqProdutos.SQL.Add('ORDER BY PRO.GRUPO, PRO.CST,PRO.DESCRICAO');

        dmMProvider.cdsProdutos.Open;
        dmMProvider.cdsProdutos.ProviderName := '';

      end;
   20:begin //rlTabCstICMSTrib
        ...
        dmDBEXMaster.fdqProdutos.SQL.Add('ORDER BY PRO.CST,PRO.DESCRICAO');
  ...
  case pRelatorio of
   ...
   14:begin
    ...
    15:begin
        ...
        dmRelatorios.fdqVendaProdutoCFOP.SQL.Add('left join cfop cfop');
        dmRelatorios.fdqVendaProdutoCFOP.SQL.Add('on cfop.cfop = mov.cfop');
        dmRelatorios.fdqVendaProdutoCFOP.SQL.Add('left JOIN PRODUTOFILIAL PF');
...
procedure TfrmRelatorios.MontarSQL_ListagemNFEntrada;
var
  sWhere:string;
begin
  ...
  dmRelatorios.cdsListNFEntrada.Close;
  dmRelatorios.cdsListNFEntrada.ProviderName := 'dspListNFEntrada';

  dmRelatorios.fdqListNFEntrada.SQL.Clear;
  dmRelatorios.fdqListNFEntrada.SQL.Add('SELECT NFENTRADA.NOTAFISCAL, NFENTRADA.ENTRADA, NFENTRADA.EMISSAO, NFENTRADA.FORNECEDOR,');
  dmRelatorios.fdqListNFEntrada.SQL.Add('FORNECEDOR.RAZAOSOCIAL, NFENTRADA.VALORDANOTA, nfentrada.baseicms, nfentrada.valoricms,');
  dmRelatorios.fdqListNFEntrada.SQL.Add('nfentrada.valordoseguro, nfentrada.valordofrete Frete_agregado, nfentrada.outrasdespesas,');
  dmRelatorios.fdqListNFEntrada.SQL.Add('nfentrada.valordoipi, nfentrada.basesubstituicao, nfentrada.valorsubstituicao ,');
  dmRelatorios.fdqListNFEntrada.SQL.Add('coalesce(c_frt.valor_total,0) CH_Frete, NFENTRADA.STATUS,');
  dmRelatorios.fdqListNFEntrada.SQL.Add('coalesce(nfentrada.finalidade_nf,-1) FINALIDADE_NF,nfentrada.OBSERVACAO_FINALIDADE');
  dmRelatorios.fdqListNFEntrada.SQL.Add('FROM NFENTRADA NFENTRADA');
  dmRelatorios.fdqListNFEntrada.SQL.Add('JOIN FORNECEDOR FORNECEDOR');
  dmRelatorios.fdqListNFEntrada.SQL.Add('ON (FORNECEDOR.FORNECEDOR = NFENTRADA.FORNECEDOR)');
  dmRelatorios.fdqListNFEntrada.SQL.Add('left join conhecimento_frete c_frt');
  dmRelatorios.fdqListNFEntrada.SQL.Add('on (c_frt.nf_entrada = nfentrada.nfentrada)');

  case rdglDataListNFEntrada.ItemIndex of
    0:sWhere := 'where nfentrada.entrada between ' + QuotedStr(FormatDateTime('mm/dd/yy',dtpDataIniListNFEntrada.Date)) + ' and ' + QuotedStr(FormatDateTime('mm/dd/yy',dtpDataFinListNFEntrada.Date));
    1:sWhere := 'where nfentrada.emissao between ' + QuotedStr(FormatDateTime('mm/dd/yy',dtpDataIniListNFEntrada.Date)) + ' and ' + QuotedStr(FormatDateTime('mm/dd/yy',dtpDataFinListNFEntrada.Date));
  end;

  if rdgFinalidadeNF.ItemIndex < 3 then
    sWhere := sWhere + ' and nfentrada.finalidade_nf = ' + IntToStr(rdgFinalidadeNF.ItemIndex);

  if Length(trim(edtFornecedorListagemNFEntrada.Text)) > 0 then
    sWhere := sWhere + ' and nfentrada.fornecedor = ' + trim(edtFornecedorListagemNFEntrada.Text);

  if cboCompradorListNFEntrada.KeyValue > 0 then
    sWhere := sWhere + ' and nfentrada.comprador = ' + IntToStr(cboCompradorListNFEntrada.KeyValue);

  dmRelatorios.fdqListNFEntrada.SQL.Add(sWhere);

  case rdgOrdenecaoLisNFEntrada.ItemIndex of
    0:dmRelatorios.fdqListNFEntrada.SQL.Add('order by fornecedor.razaosocial');
    1:dmRelatorios.fdqListNFEntrada.SQL.Add('order by nfentrada.entrada');
    2:dmRelatorios.fdqListNFEntrada.SQL.Add('order by nfentrada.emissao');
    3:dmRelatorios.fdqListNFEntrada.SQL.Add('order by nfentrada.entrada, nfentrada.entrada ');
  end;

  dmRelatorios.cdsListNFEntrada.Open;
  dmRelatorios.cdsListNFEntrada.ProviderName := '';

end;

procedure TfrmRelatorios.MontarSQL_ListagemNFSaida;
var
  sWhere:string;
begin
  ...
  case pOpcao of
    0:begin
        ...
        dmRelatorios.cdsListNFsaida.Close;
        dmRelatorios.cdsListNFsaida.ProviderName := 'dspListNFsaida';

        dmRelatorios.fdqListNFsaida.SQL.Clear;
        dmRelatorios.fdqListNFsaida.SQL.Add('SELECT NFS.EMISSAO, NFS.NFSAIDA, NFS.DESTINATARIO, NFS.DESTINATARIO_,');
        dmRelatorios.fdqListNFsaida.SQL.Add('NFS.VALORDANOTA, NFS.STATUS, NFS.CANCELADA, CLI.RAZAOSOCIAL');
        dmRelatorios.fdqListNFsaida.SQL.Add('FROM NFSAIDA NFS');
        dmRelatorios.fdqListNFsaida.SQL.Add('JOIN CLIENTES CLI');
        dmRelatorios.fdqListNFsaida.SQL.Add('ON(CLI.CLIENTE = NFS.DESTINATARIO)');

        sWhere := 'where nfs.emissao between ' + QuotedStr(FormatDateTime('mm/dd/yy',dtpDataIniNFS.Date)) + ' and ' + QuotedStr(FormatDateTime('mm/dd/yy',dtpDataFinNFS.Date));

        if rdgStatusNFS.ItemIndex <= 3 then
          sWhere := sWhere + ' and nfs.status = ' + IntToStr(rdgStatusNFS.ItemIndex);

        if Length(trim(edtCNPJClienteNFS.Text)) > 0 then
          sWhere := sWhere + ' and nfs.destinatario = ' + trim(edtCNPJClienteNFS.Text);

        dmRelatorios.fdqListNFsaida.SQL.Add(sWhere);

        dmRelatorios.fdqListNFsaida.SQL.Add('ORDER BY NFS.NFSAIDA');

        dmRelatorios.cdsListNFsaida.Open;
        dmRelatorios.cdsListNFsaida.ProviderName := '';

      end;
    1:begin  //totaliza relatorio

        dmRelatorios.cdsTotCFOP_NFS.Close;
        dmRelatorios.cdsTotCFOP_NFS.ProviderName := 'dspTotCFOP_NFS';

        dmRelatorios.fdqTotCFOP_NFS.SQL.Clear;
        dmRelatorios.fdqTotCFOP_NFS.SQL.Add('SELECT NFSAIDA, CFOP, SUM(COALESCE(sub_total - valor_desconto,0)) AS ST');
        dmRelatorios.fdqTotCFOP_NFS.SQL.Add('FROM itemnfsaida ITNFS');

        sWhere := 'where nfsaida in (select nfsaida from nfsaida where emissao between ' + QuotedStr(FormatDateTime('mm/dd/yy',dtpDataIniNFS.Date)) + ' and ' + QuotedStr(FormatDateTime('mm/dd/yy',dtpDataFinNFS.Date));

        if rdgStatusNFS.ItemIndex <= 3 then
          sWhere := sWhere + ' and status = ' + IntToStr(rdgStatusNFS.ItemIndex);

        if Length(trim(edtCNPJClienteNFS.Text)) > 0 then
          sWhere := sWhere + ' and destinatario = ' + trim(edtCNPJClienteNFS.Text);

        sWhere := sWhere + ')';

        dmRelatorios.fdqTotCFOP_NFS.SQL.Add(sWhere);
        dmRelatorios.fdqTotCFOP_NFS.SQL.Add('GROUP BY NFSAIDA, CFOP');
        dmRelatorios.fdqTotCFOP_NFS.SQL.Add('ORDER BY NFSAIDA');

        dmRelatorios.cdsTotCFOP_NFS.Open;
        dmRelatorios.cdsTotCFOP_NFS.ProviderName := '';

      end;
    2:begin  //totaliza cfop nfs

        dmRelatorios.cdsTotGeralCFOP_NFS.Close;
        dmRelatorios.cdsTotGeralCFOP_NFS.ProviderName := 'dspTotGeralCFOP_NFS';

        dmRelatorios.fdqTotGeralCFOP_NFS.SQL.Clear;
        dmRelatorios.fdqTotGeralCFOP_NFS.SQL.Add('SELECT CFOP, SUM(COALESCE(sub_total - valor_desconto,0)) AS ST');
        dmRelatorios.fdqTotGeralCFOP_NFS.SQL.Add('FROM itemnfsaida ITNFS');
        ...
        dmRelatorios.fdqTotGeralCFOP_NFS.SQL.Add(sWhere);

        dmRelatorios.fdqTotGeralCFOP_NFS.SQL.Add('GROUP BY CFOP');
        dmRelatorios.fdqTotGeralCFOP_NFS.SQL.Add('order by cfop');

        dmRelatorios.cdsTotGeralCFOP_NFS.Open;
        dmRelatorios.cdsTotGeralCFOP_NFS.ProviderName := '';
...
begin
  ...
  case pRelatorio of
   ...
   14:begin //apoio a efd contribuições
   ...
   19,20:begin // tabela produto de GRUPO x CST x TRIBUTAÇÃO
...
procedure TfrmRelatorios.tbsListagemNFEntradaShow(Sender: TObject);
begin

  dtpDataIniListNFEntrada.Date := StartOfTheMonth(Date);
  dtpDataFinListNFEntrada.Date := EndOfTheMonth(Date);
...
procedure TfrmRelatorios.tbsListagemNFSaidaShow(Sender: TObject);
begin

  dtpDataIniNFS.Date := StartOfTheMonth(Date);
  dtpDataFinNFS.Date := EndOfTheMonth(Date);

  edtCNPJClienteNFS.SetFocus;

end;

procedure TfrmRelatorios.tbsRelPedidosShow(Sender: TObject);
begin

  dtpDataIniPedidoVenda.Date    := Date;
  dtpDataFinalPedidoVenda.Date  := dtpDataIniPedidoVenda.Date;
