    ...
    0:begin
        ...
        begin
          ...
          begin
            ...
            //armazena percentual de ipi em variável
...
begin
  ...
  case cdsCFOP_CFPSNATUREZA_CFOP.Value of
    ...
    2:cdsCFOP_CFPSDESCRICAO_NATUREZA.Value := 'DEVOLUÇÃO';
    3:cdsCFOP_CFPSDESCRICAO_NATUREZA.Value := 'FRETE';
    4:cdsCFOP_CFPSDESCRICAO_NATUREZA.Value := 'SERVIÇO';
    5:cdsCFOP_CFPSDESCRICAO_NATUREZA.Value := 'OUTROS';
  else
    cdsCFOP_CFPSDESCRICAO_NATUREZA.Value := 'NÃO CONFIGURADA';
  end;

  case cdsCFOP_CFPSOPERACAO.Value of
    0:cdsCFOP_CFPSDESCRICAO_OPERACAO.Value := 'ENTRADA';
    1:cdsCFOP_CFPSDESCRICAO_NATUREZA.Value := 'SAÍDA';
    2:cdsCFOP_CFPSDESCRICAO_NATUREZA.Value := 'IMPLAMTAÇÃO';
    3:cdsCFOP_CFPSDESCRICAO_NATUREZA.Value := 'SEM MOVIMENTAÇÃO';
  else
    cdsCFOP_CFPSDESCRICAO_NATUREZA.Value := 'NÃO CONFIGURADA';
  end;
...
begin
  ...
  case cdsCtrlNotasTIPO_LANCAMENTO.Value of
    0:cdsCtrlNotasS_TIPO_LANCAMENTO.Value := 'LANÇAMENTO NORMAL';
    1:cdsCtrlNotasS_TIPO_LANCAMENTO.Value := 'UTILIZADO FUNÇÃO PARA DUPLICAÇÃO';
  end;
...
begin
  ...
  case cdsFCP_EstadosEXCECAO.Value of
    0:cdsFCP_EstadosS_EXCECAO.Value := 'NÃO';
...
begin
  ...
  case cdsItemCaixaLojaTIPO_LANCAMENTO.Value of
    0,1:cdsItemCaixaLojaS_TIPO_LANCAMENTO.Value := 'CARTÕES';
...
begin
  ...
  case cdsItens_OS_PecasTIPO.Value of
    ...
    4:cdsItens_OS_PecasDESCRICAO_TIPO.Value := 'PRODUÇÃO';
...
begin
  ...
  case cdsItens_OS_servicoTIPO.Value of
    0:cdsItens_OS_servicoDESCRICAO_TIPO.Value := 'Padrão';
...
begin
  ...
  case cdsLogSysOP_OPERACAO.Value of
    0:cdsLogSysS_OPERACAO.Value := 'INCLUSÃO';
    1:cdsLogSysS_OPERACAO.Value := 'ALTERAÇÃO';
    2:cdsLogSysS_OPERACAO.Value := 'EXCLUSÃO';
    3:cdsLogSysS_OPERACAO.Value := 'AÇÃO';
...
begin
  ...
  cdsNFSaidaVALORDOIPI.Value                      := 0;
  cdsNFSaidaVALORDANOTA.Value                     := 0;
  {OBRIGAR O USUÁRIO A ESCOLHER O TIPO DE FRETE}
...
//  cdsNFSaidaC_M_C: TStringField
  ...
  cdsNFSaidaFINALIDADE_NF.Value                   := 0; { 0-Normal, 1-Complementar, 2-Ajuste, 3-Devolução}
  cdsNFSaidaCANCELAMENTO_ESTEMPORANEO.Value       := 0; {0-Não / 1-Sim}
...
//  cdsNFSaidaCONSUMIDOR_FINAL.Value                := 0; {0-NÃO / 1-SIM}
  cdsNFSaidaDESTINO_OPERACAO.Value                := 0; { 1-Interna / 2-Interestadual / 3-Exterior }
  cdsNFSaidaLOCAL_RETIRADA.Value                  := 0; {0-não é diferete do emitene / 1-é diferente do emitente}
  cdsNFSaidaLOCAL_ENTREGA.Value                   := 0; {0-não é diferete do destinatário / 1-é diferente do destinatário}
  cdsNFSaidaTOTAL_ICMS_DESONERADO.Value           := 0;
  cdsNFSaidaTRANSPORTADOR_ISENTO_ICMS.Value       := 0; {0-NÃO / 1-SIM}
  ...
  if (UpperCase(dmMProvider.cdsPesqClientesINSCRICAO.AsString) = 'ISENTO') and (dmMProvider.cdsPesqClientesESTADOENTREGA.Value <> dmMProvider.cdsFilialESTADO.Value)then
    dmMProvider.cdsNFSaidaINDICADOR_IE_DESTINATARIO.Value := 9  { não contribuinte}
...
begin
  ...
  case cdsParamIntegraPDVMODELO_PDV.Value of
    4:begin
        ...
        cdsParamIntegraPDVPASTA_EXPORTACAO.Value                  := 'C:\DJSYSTEM\MONITOR\Exportar\'; //verificar instalação dj pdv
        cdsParamIntegraPDVPASTA_IMPORTACAO.Value                  := 'C:\DJSYSTEM\MONITOR\Importar\'; //verificar instalação dj pdv
...
begin
  ...
  case cdsPedidoZeusPEDIDO_ORCAMENTO.Value of
    0:cdsPedidoZeusS_ORCAMENTO_PEDIDO.Value := 'ORÇAMENTO';
...
begin
  ...
  case cdsProdutosATIVO.Value of
    0:cdsProdutosS_ATIVO.Value := 'NÃO';
    1:cdsProdutosS_ATIVO.Value := 'SIM';
  end;

  case cdsProdutosPROMOCAO.Value of
    0:cdsProdutosS_PROMOCAO.Value := 'NÃO';
