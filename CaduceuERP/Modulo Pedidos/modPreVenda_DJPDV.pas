...
begin
  ...
  try
    ...
    if dmMProvider.cdsPedidoZeusCLIENTE.Value > 0 then
      ...
      sLinha := sLinha + '0|'
                  + Trim(dmMProvider.cdsEndEntregaPedZeusNOME_CLIENTE.Value) + '|'
                  +  '||'
    else
      sLinha := sLinha + '0|||0|';

    sLinha := sLinha + FormatFloat('###0.00',dmMProvider.cdsPedidoZeusVALOR_DO_PEDIDO.Value) + '|'
                + FormatFloat('###0.00',0{dmMProvider.cdsPedidoZeusVALOR_DESCONTO.Value}) + '|'
                + FormatFloat('###0.00',0{dmMProvider.cdsPedidoZeusVALOR_ACRESCIMO.Value}) + '|'
    ...
    case dmMProvider.cdsParamIntegraPDVTIPO_CODIGO_INTEGRACAO.Value of
      ...
      1:begin
          ...
          begin
            ...
            sLinha := 'PIT|' + IntToStr(dmMProvider.cdsItemPedZeus.RecNo) + '|'
                        + FormatFloat('000000',dmMProvider.cdsItemPedZeusPRODUTO.Value) + '|'
                        + FormatFloat('###0.000',dmMProvider.cdsItemPedZeusQUANTIDADE.Value) + '|'
                        + FormatFloat('###0.000',dmMProvider.cdsItemPedZeusVALOR_UNITARIO.Value) + '|'
                        + FormatFloat('###0.00',dmMProvider.cdsItemPedZeusVALOR_DESCONTO.Value) + '|'
                        + FormatFloat('###0.00',dmMProvider.cdsItemPedZeusVALOR_ACRESCIMO.Value) + '|'
                        + FormatFloat('###0.000',(dmMProvider.cdsItemPedZeusVALOR_TOTAL_ITEM.Value - dmMProvider.cdsItemPedZeusVALOR_DESCONTO.Value) + dmMProvider.cdsItemPedZeusVALOR_ACRESCIMO.Value) + '|'
