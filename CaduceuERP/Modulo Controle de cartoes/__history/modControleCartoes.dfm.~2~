object frmModControleCartoes: TfrmModControleCartoes
  Left = 0
  Top = 0
  BorderIcons = [biSystemMenu, biMinimize]
  BorderStyle = bsSingle
  ClientHeight = 506
  ClientWidth = 737
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  Position = poScreenCenter
  OnCreate = FormCreate
  OnKeyDown = FormKeyDown
  OnKeyPress = FormKeyPress
  PixelsPerInch = 96
  TextHeight = 13
  object imgSair: TImage
    Left = 670
    Top = 454
    Width = 51
    Height = 44
    Cursor = crHandPoint
    Hint = 'Sair'
    ParentShowHint = False
    Picture.Data = {
      }
    ShowHint = True
    Stretch = True
    OnClick = WindowClose1Execute
  end
  object imgEditar: TImage
    Left = 670
    Top = 242
    Width = 51
    Height = 44
    Cursor = crHandPoint
    Hint = 'Editar'
    ParentShowHint = False
    Picture.Data = {
      
      82}
    ShowHint = True
    Stretch = True
    OnClick = actEditarExecute
  end
  object imgExcluir: TImage
    Left = 670
    Top = 187
    Width = 51
    Height = 44
    Cursor = crHandPoint
    Hint = 'Excluir'
    ParentShowHint = False
    Picture.Data = {
      
      6082}
    ShowHint = True
    Stretch = True
    OnClick = lblF5Click
  end
  object imgSalvar: TImage
    Left = 670
    Top = 134
    Width = 51
    Height = 44
    Cursor = crHandPoint
    Hint = 'Salvar'
    ParentShowHint = False
    Picture.Data = {
      
      AE426082}
    ShowHint = True
    Stretch = True
    OnClick = actSalvarExecute
  end
  object imgDesfazer: TImage
    Left = 670
    Top = 80
    Width = 51
    Height = 44
    Cursor = crHandPoint
    Hint = 'Desfazer'
    ParentShowHint = False
    Picture.Data = {
      
      4E44AE426082}
    ShowHint = True
    Stretch = True
    OnClick = actDesfazerExecute
  end
  object imgNovo: TImage
    Left = 670
    Top = 26
    Width = 51
    Height = 44
    Cursor = crHandPoint
    Hint = 'Incluir'
    ParentShowHint = False
    Picture.Data = {
      
      426082}
    ShowHint = True
    Stretch = True
    OnClick = actIncluirExecute
  end
  object lblF6: TLabel
    Left = 690
    Top = 258
    Width = 12
    Height = 13
    Cursor = crHandPoint
    Hint = 'Editar'
    Caption = 'F6'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlack
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    OnClick = actEditarExecute
  end
  object lblF5: TLabel
    Left = 690
    Top = 200
    Width = 12
    Height = 13
    Cursor = crHandPoint
    Hint = 'Excluir'
    Caption = 'F5'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlack
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    OnClick = lblF5Click
  end
  object lblF4: TLabel
    Left = 690
    Top = 143
    Width = 12
    Height = 13
    Cursor = crHandPoint
    Hint = 'Salvar'
    Caption = 'F4'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlack
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    OnClick = actSalvarExecute
  end
  object lblF3: TLabel
    Left = 690
    Top = 88
    Width = 12
    Height = 13
    Cursor = crHandPoint
    Hint = 'Desfazer'
    Caption = 'F3'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWhite
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    OnClick = actDesfazerExecute
  end
  object lblF2: TLabel
    Left = 690
    Top = 42
    Width = 12
    Height = 13
    Cursor = crHandPoint
    Hint = 'Incluir'
    Caption = 'F2'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlack
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    OnClick = actIncluirExecute
  end
  object imgBaixa: TImage
    Left = 670
    Top = 341
    Width = 51
    Height = 44
    Cursor = crHandPoint
    Hint = 'Editar'
    ParentShowHint = False
    Picture.Data = {
      
      E12C1AF3039BF4B5D03B9510770000000049454E44AE426082}
    ShowHint = True
    Stretch = True
    OnClick = actBaixaCartaoExecute
  end
  object lblF7: TLabel
    Left = 690
    Top = 357
    Width = 12
    Height = 13
    Cursor = crHandPoint
    Hint = 'Editar'
    Caption = 'F7'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlack
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    OnClick = actBaixaCartaoExecute
  end
  object grpMensagem: TGroupBox
    Left = 8
    Top = 461
    Width = 649
    Height = 38
    Caption = 'Mensagem'
    TabOrder = 0
    object lblMsg: TLabel
      Left = 66
      Top = 14
      Width = 518
      Height = 16
      ParentCustomHint = False
      AutoSize = False
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -12
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
    end
  end
  object pgcControleCartoes: TPageControl
    Left = 8
    Top = 77
    Width = 649
    Height = 381
    ActivePage = tbsCadastro
    TabOrder = 1
    object tbsTabela: TTabSheet
      Caption = 'Tabela'
      object grdTabela: TDBGrid
        Left = 0
        Top = 0
        Width = 641
        Height = 353
        Align = alClient
        DataSource = dmMSource.dtsCtrlCartoes
        Options = [dgTitles, dgIndicator, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit, dgTitleClick, dgTitleHotTrack]
        ReadOnly = True
        TabOrder = 0
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -11
        TitleFont.Name = 'Tahoma'
        TitleFont.Style = []
        OnDrawColumnCell = grdTabelaDrawColumnCell
        OnDblClick = grdTabelaDblClick
        OnKeyPress = grdTabelaKeyPress
        Columns = <
          item
            Expanded = False
            FieldName = 'EMISSAO'
            Title.Alignment = taCenter
            Title.Caption = 'EMISS'#195'O'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'NOME_ADM'
            Title.Alignment = taCenter
            Title.Caption = 'ADMINISTRADORA'
            Width = 275
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'VALORDACOMPRA'
            Title.Alignment = taCenter
            Title.Caption = 'VALOR OPERA'#199#195'O'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'DESCR_STATUS'
            Title.Alignment = taCenter
            Title.Caption = 'STATUS'
            Width = 76
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'DESCR_TIPO_OPER'
            Title.Alignment = taCenter
            Title.Caption = 'DEB./CRED.'
            Visible = True
          end>
      end
    end
    object tbsCadastro: TTabSheet
      Caption = 'Cadastro'
      ImageIndex = 1
      object grpCadastro: TGroupBox
        Left = 0
        Top = 0
        Width = 641
        Height = 353
        Align = alClient
        Enabled = False
        TabOrder = 0
        object lblControle: TLabel
          Left = 16
          Top = 21
          Width = 41
          Height = 13
          Caption = 'Controle'
        end
        object lblAdministradora: TLabel
          Left = 16
          Top = 45
          Width = 72
          Height = 13
          Caption = 'Administradora'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clRed
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
        end
        object lblEmissao: TLabel
          Left = 16
          Top = 69
          Width = 38
          Height = 13
          Caption = 'Emiss'#227'o'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clRed
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
        end
        object lblDeposito: TLabel
          Left = 16
          Top = 93
          Width = 42
          Height = 13
          Caption = 'Dep'#243'sito'
        end
        object lblRecebimento: TLabel
          Left = 16
          Top = 117
          Width = 62
          Height = 13
          Caption = 'Recebimento'
        end
        object grpTotais: TGroupBox
          Left = 2
          Top = 259
          Width = 637
          Height = 92
          Align = alBottom
          Caption = 'Totais do per'#237'odo'
          Enabled = False
          TabOrder = 0
          object grpCreditos: TGroupBox
            Left = 15
            Top = 16
            Width = 300
            Height = 69
            Caption = 'Cr'#233'ditos'
            TabOrder = 0
            object lblCreditoAberto: TLabel
              Left = 85
              Top = 20
              Width = 49
              Height = 13
              Caption = 'Em aberto'
            end
            object lblCreditoBaixado: TLabel
              Left = 85
              Top = 43
              Width = 43
              Height = 13
              Caption = 'Baixados'
            end
            object edtCreditoAberto: TDBEdit
              Left = 141
              Top = 17
              Width = 121
              Height = 21
              Color = 14079702
              DataField = 'OP_TOTAL_CR_ABERTO'
              DataSource = dmMSource.dtsTotaisCtrlCartaoes
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWindowText
              Font.Height = -11
              Font.Name = 'Tahoma'
              Font.Style = [fsBold]
              ParentFont = False
              TabOrder = 0
              OnKeyDown = edtControleKeyDown
              OnKeyPress = edtControleKeyPress
            end
            object dtCreditoBaixado: TDBEdit
              Left = 141
              Top = 40
              Width = 121
              Height = 21
              Color = 14079702
              DataField = 'OP_TOTAL_CR_BAIXADO'
              DataSource = dmMSource.dtsTotaisCtrlCartaoes
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWindowText
              Font.Height = -11
              Font.Name = 'Tahoma'
              Font.Style = [fsBold]
              ParentFont = False
              TabOrder = 1
              OnKeyDown = edtControleKeyDown
              OnKeyPress = edtControleKeyPress
            end
          end
          object grpDebitos: TGroupBox
            Left = 327
            Top = 16
            Width = 300
            Height = 69
            Caption = 'D'#233'bitos'
            TabOrder = 1
            object lblDebitoAberto: TLabel
              Left = 78
              Top = 19
              Width = 49
              Height = 13
              Caption = 'Em aberto'
            end
            object lblDebitoBaixado: TLabel
              Left = 78
              Top = 42
              Width = 43
              Height = 13
              Caption = 'Baixados'
            end
            object edtDebitoAberto: TDBEdit
              Left = 150
              Top = 16
              Width = 121
              Height = 21
              Color = 14079702
              DataField = 'OP_TOTAL_DEB_ABERTO'
              DataSource = dmMSource.dtsTotaisCtrlCartaoes
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWindowText
              Font.Height = -11
              Font.Name = 'Tahoma'
              Font.Style = [fsBold]
              ParentFont = False
              TabOrder = 0
              OnKeyDown = edtControleKeyDown
              OnKeyPress = edtControleKeyPress
            end
            object edtDebitobaixado: TDBEdit
              Left = 150
              Top = 39
              Width = 121
              Height = 21
              Color = 14079702
              DataField = 'OP_TOTAL_DEB_BAIXADO'
              DataSource = dmMSource.dtsTotaisCtrlCartaoes
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWindowText
              Font.Height = -11
              Font.Name = 'Tahoma'
              Font.Style = [fsBold]
              ParentFont = False
              TabOrder = 1
              OnKeyDown = edtControleKeyDown
              OnKeyPress = edtControleKeyPress
            end
          end
        end
        object edtControle: TDBEdit
          Left = 92
          Top = 18
          Width = 82
          Height = 21
          Color = 14079702
          DataField = 'CONTROLECARTAOTICKET'
          DataSource = dmMSource.dtsCtrlCartoes
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = [fsBold]
          ParentFont = False
          ReadOnly = True
          TabOrder = 1
          OnKeyDown = edtControleKeyDown
          OnKeyPress = edtControleKeyPress
        end
        object cboAdministradora: TDBLookupComboBox
          Left = 94
          Top = 41
          Width = 356
          Height = 21
          DataField = 'ADMINISTRADORA'
          DataSource = dmMSource.dtsCtrlCartoes
          KeyField = 'ADMINISTRADORA'
          ListField = 'NOME'
          ListSource = dmMSource.dtsAdministradoras
          TabOrder = 2
          OnKeyPress = edtControleKeyPress
        end
        object edtDataEmissao: TDBEdit
          Left = 92
          Top = 66
          Width = 93
          Height = 21
          DataField = 'EMISSAO'
          DataSource = dmMSource.dtsCtrlCartoes
          TabOrder = 3
          OnEnter = edtDataEmissaoEnter
          OnExit = edtDataEmissaoExit
          OnKeyDown = edtControleKeyDown
          OnKeyPress = edtControleKeyPress
        end
        object edtDataDeposito: TDBEdit
          Left = 92
          Top = 90
          Width = 93
          Height = 21
          DataField = 'DEPOSITO'
          DataSource = dmMSource.dtsCtrlCartoes
          TabOrder = 4
          OnEnter = edtDataEmissaoEnter
          OnExit = edtDataDepositoExit
          OnKeyDown = edtControleKeyDown
          OnKeyPress = edtControleKeyPress
        end
        object edtdataRecebimento: TDBEdit
          Left = 92
          Top = 114
          Width = 93
          Height = 21
          DataField = 'RECEBIMENTO'
          DataSource = dmMSource.dtsCtrlCartoes
          TabOrder = 5
          OnEnter = edtDataEmissaoEnter
          OnExit = edtDataDepositoExit
          OnKeyDown = edtControleKeyDown
          OnKeyPress = edtControleKeyPress
        end
        object grpValores: TGroupBox
          Left = 16
          Top = 141
          Width = 432
          Height = 105
          Caption = 'Valores'
          TabOrder = 6
          object lblValorOperacao: TLabel
            Left = 16
            Top = 24
            Width = 72
            Height = 13
            Caption = 'Valor opera'#231#227'o'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clRed
            Font.Height = -11
            Font.Name = 'Tahoma'
            Font.Style = []
            ParentFont = False
          end
          object lblValorDesconto: TLabel
            Left = 16
            Top = 48
            Width = 71
            Height = 13
            Caption = 'Valor desconto'
          end
          object lblValorReceber: TLabel
            Left = 16
            Top = 72
            Width = 73
            Height = 13
            Caption = 'Valor a receber'
          end
          object edtValorOperacao: TDBEdit
            Left = 112
            Top = 21
            Width = 121
            Height = 21
            DataField = 'VALORDACOMPRA'
            DataSource = dmMSource.dtsCtrlCartoes
            TabOrder = 0
            OnEnter = edtDataEmissaoEnter
            OnExit = edtValorOperacaoExit
            OnKeyDown = edtControleKeyDown
            OnKeyPress = edtValorOperacaoKeyPress
          end
          object edtValorDesconto: TDBEdit
            Left = 112
            Top = 45
            Width = 121
            Height = 21
            Color = 14079702
            DataField = 'VALORDODESCONTO'
            DataSource = dmMSource.dtsCtrlCartoes
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'Tahoma'
            Font.Style = [fsBold]
            ParentFont = False
            ReadOnly = True
            TabOrder = 1
            OnKeyDown = edtControleKeyDown
            OnKeyPress = edtValorOperacaoKeyPress
          end
          object edtvalorReceber: TDBEdit
            Left = 112
            Top = 69
            Width = 121
            Height = 21
            Color = 14079702
            DataField = 'VALORARECEBER'
            DataSource = dmMSource.dtsCtrlCartoes
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'Tahoma'
            Font.Style = [fsBold]
            ParentFont = False
            ReadOnly = True
            TabOrder = 2
            OnKeyDown = edtControleKeyDown
            OnKeyPress = edtValorOperacaoKeyPress
          end
        end
        object rdgTipoOperacao: TDBRadioGroup
          Left = 469
          Top = 21
          Width = 131
          Height = 78
          Caption = 'Tipo opera'#231#227'o'
          Columns = 2
          DataField = 'TIPO_OPERACAO'
          DataSource = dmMSource.dtsCtrlCartoes
          Items.Strings = (
            'Cr'#233'dito'
            'D'#233'bito')
          TabOrder = 7
          Values.Strings = (
            '0'
            '1')
        end
        object grpStatus: TGroupBox
          Left = 469
          Top = 105
          Width = 131
          Height = 141
          Caption = 'Status'
          Enabled = False
          TabOrder = 8
          object edtDescrStatus: TDBEdit
            Left = 14
            Top = 24
            Width = 106
            Height = 21
            CharCase = ecUpperCase
            DataField = 'DESCR_STATUS'
            DataSource = dmMSource.dtsCtrlCartoes
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'Tahoma'
            Font.Style = [fsBold]
            ParentFont = False
            TabOrder = 0
            OnChange = edtDescrStatusChange
            OnKeyDown = edtControleKeyDown
            OnKeyPress = edtControleKeyPress
          end
          object grpDatabaixa: TGroupBox
            Left = 7
            Top = 72
            Width = 119
            Height = 54
            Caption = 'Data baixa'
            TabOrder = 1
            object edtDataBaixa: TDBEdit
              Left = 8
              Top = 24
              Width = 107
              Height = 21
              DataField = 'BAIXA'
              DataSource = dmMSource.dtsCtrlCartoes
              TabOrder = 0
              OnKeyDown = edtControleKeyDown
              OnKeyPress = edtControleKeyPress
            end
          end
        end
      end
    end
  end
  object grpPesquisa: TGroupBox
    Left = 8
    Top = 1
    Width = 649
    Height = 70
    Caption = 'Filtro'
    TabOrder = 2
    object rdpOpcoesPesquisa: TRadioGroup
      Left = 0
      Top = 18
      Width = 269
      Height = 48
      Cursor = crHandPoint
      Caption = 'Op'#231#245'es'
      Columns = 2
      ItemIndex = 0
      Items.Strings = (
        'Data emissao'
        'Data deposito'
        'Data baixa')
      TabOrder = 0
      OnClick = rdpOpcoesPesquisaClick
    end
    object grpConteudoPesquisa: TGroupBox
      Left = 270
      Top = 19
      Width = 375
      Height = 48
      Caption = 'Conte'#250'do'
      TabOrder = 1
      object impPesquisar: TImage
        Left = 340
        Top = 16
        Width = 27
        Height = 22
        Cursor = crHandPoint
        Hint = 'pesquisar'
        ParentShowHint = False
        Picture.Data = {
          
          00000049454E44AE426082}
        ShowHint = True
        Stretch = True
        OnClick = impPesquisarClick
      end
      object ldoFiltroInicial: TLabel
        Left = 31
        Top = 21
        Width = 27
        Height = 13
        Caption = 'Inicial'
      end
      object lblFiltroFinal: TLabel
        Left = 189
        Top = 22
        Width = 22
        Height = 13
        Caption = 'Final'
      end
      object dtpInicial: TDateTimePicker
        Left = 73
        Top = 16
        Width = 97
        Height = 21
        Date = 41842.577148206020000000
        Time = 41842.577148206020000000
        TabOrder = 0
      end
      object dtpFinal: TDateTimePicker
        Left = 226
        Top = 16
        Width = 97
        Height = 21
        Date = 41842.577148206020000000
        Time = 41842.577148206020000000
        TabOrder = 1
      end
    end
  end
  object ActionList1: TActionList
    Left = 380
    Top = 289
    object actEditar: TAction
      Category = 'Window'
      Caption = 'actEditar'
      ShortCut = 117
      OnExecute = actEditarExecute
    end
    object actDesfazer: TAction
      Category = 'Window'
      Caption = 'actDesfazer'
      ShortCut = 114
      OnExecute = actDesfazerExecute
    end
    object WindowClose1: TWindowClose
      Category = 'Window'
      Caption = 'C&lose'
      Enabled = False
      Hint = 'Close'
      OnExecute = WindowClose1Execute
    end
    object actMinimizar: TAction
      Category = 'Window'
      Caption = 'actMinimizar'
    end
    object actIncluir: TAction
      Category = 'Window'
      Caption = 'actIncluir'
      ShortCut = 113
      OnExecute = actIncluirExecute
    end
    object actSalvar: TAction
      Category = 'Window'
      Caption = 'actSalvar'
      ShortCut = 115
      OnExecute = actSalvarExecute
    end
    object actExcluir: TAction
      Category = 'Window'
      Caption = 'actExcluir'
      OnExecute = actExcluirExecute
    end
    object actBaixaCartao: TAction
      Category = 'Window'
      Caption = 'actBaixaCartao'
      ShortCut = 118
      OnExecute = actBaixaCartaoExecute
    end
  end
end
