...
begin
  ...
  memResultado.Clear;
  memResultado.Lines.Add('Itens que sofrerão movimentação de seus saldos');
  memResultado.Lines.Add('--------------------------------------------------------------------------');
  memResultado.Lines.Add('[ OPERAÇÃO ] [ QUANTIDADE ] [PRODUTO/DESCRIÇÃO]');
  ...
  begin
    ...
    memResultado.Lines.Add('  SAÍDA     ' + format('%14.14s', [formatfloat('#,##0.000', dmMProvider.cdsItemOrdemProducaoQUANTIDADE_PREVISTA.Value)]) + '   ' + format('%7.7s',[FormatFloat('#######',dmMProvider.cdsItemOrdemProducaoPRODUTO_INSUMO.Value)]) + ' ' + dmMProvider.cdsItemOrdemProducaoDESCRICAO_PRODUTO.Value);
