object frmCCe: TfrmCCe
  Left = 0
  Top = 0
  BorderIcons = [biSystemMenu, biMinimize]
  BorderStyle = bsSingle
  Caption = 'Carta de Corre'#231#227'o Eletr'#244'nica - CC-e'
  ClientHeight = 490
  ClientWidth = 822
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  OnActivate = FormActivate
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  object imgSair: TImage
    Left = 768
    Top = 438
    Width = 44
    Height = 44
    Cursor = crHandPoint
    Hint = 'Sair'
    ParentShowHint = False
    Picture.Data = {
      }
    ShowHint = True
    Stretch = True
    OnClick = imgSairClick
  end
  object imgOk: TImage
    Left = 768
    Top = 13
    Width = 44
    Height = 44
    Cursor = crHandPoint
    Hint = 'Confirmar'
    ParentShowHint = False
    Picture.Data = {
      
      6082}
    ShowHint = True
    Stretch = True
    OnClick = imgOkClick
  end
  object rdgOpcaoPesquisa: TRadioGroup
    Left = 8
    Top = 8
    Width = 468
    Height = 49
    Caption = 'Op'#231#245'es para pesquisa'
    Columns = 4
    Items.Strings = (
      'N'#186' NF-e'
      'Emiss'#227'o NF-e'
      'Emiss'#227'o CC-e'
      'Todas')
    TabOrder = 0
    OnClick = rdgOpcaoPesquisaClick
  end
  object grpConteudoPesquisa: TGroupBox
    Left = 476
    Top = 8
    Width = 284
    Height = 49
    Caption = 'Conte'#250'do para pesquisa'
    TabOrder = 1
    object impPesquisar: TImage
      Left = 240
      Top = 17
      Width = 27
      Height = 22
      Cursor = crHandPoint
      Hint = 'pesquisar'
      ParentShowHint = False
      Picture.Data = {
        
        00000049454E44AE426082}
      ShowHint = True
      Stretch = True
      OnClick = edtConteudoPesquisaExit
    end
    object edtConteudoPesquisa: TEdit
      Left = 7
      Top = 18
      Width = 227
      Height = 21
      TabOrder = 0
      TextHint = 'Informe o conte'#250'do para pesquisa'
      OnEnter = edtConteudoPesquisaEnter
      OnExit = edtConteudoPesquisaExit
      OnKeyPress = edtConteudoPesquisaKeyPress
    end
  end
  object pgcCCe: TPageControl
    Left = 8
    Top = 63
    Width = 752
    Height = 386
    ActivePage = tbsCCe
    TabOrder = 2
    object tbsTabela: TTabSheet
      Caption = 'Tabela'
      ExplicitLeft = 0
      ExplicitTop = 0
      ExplicitWidth = 0
      ExplicitHeight = 0
      object grdTabela: TDBGrid
        Left = 0
        Top = 0
        Width = 744
        Height = 358
        Align = alClient
        DataSource = dmMSource.dtsCCe
        DrawingStyle = gdsClassic
        Options = [dgTitles, dgIndicator, dgColLines, dgTabs, dgConfirmDelete, dgCancelOnExit, dgTitleClick, dgTitleHotTrack]
        ReadOnly = True
        TabOrder = 0
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -11
        TitleFont.Name = 'Tahoma'
        TitleFont.Style = []
        Columns = <
          item
            Expanded = False
            FieldName = 'PROTOCOLO'
            Title.Alignment = taCenter
            Title.Caption = 'Protocolo CC-e'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'DATA_CCE'
            Title.Alignment = taCenter
            Title.Caption = 'Emiss'#227'o CC-e'
            Width = 93
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'CCE'
            Title.Alignment = taCenter
            Title.Caption = 'Evento'
            Width = 96
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'NOTA_FISCAL'
            Title.Alignment = taCenter
            Title.Caption = 'Nota Fiscal'
            Width = 92
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'EMISSAO_NOTA'
            Title.Alignment = taCenter
            Title.Caption = 'Emiss'#227'o NF'
            Width = 79
            Visible = True
          end>
      end
    end
    object tbsCCe: TTabSheet
      Caption = 'CC-e'
      ImageIndex = 1
      ExplicitLeft = 0
      ExplicitTop = 0
      ExplicitWidth = 0
      ExplicitHeight = 0
      object grpInfoNFe: TGroupBox
        Left = 1
        Top = 3
        Width = 608
        Height = 78
        Caption = 'Informa'#231#245'es da NF-e'
        TabOrder = 0
        object frpNumeroNfe: TGroupBox
          Left = 3
          Top = 24
          Width = 102
          Height = 39
          Caption = 'N'#250'mero'
          TabOrder = 0
          object lblNumneroNfe: TLabel
            Left = 5
            Top = 16
            Width = 91
            Height = 18
            Alignment = taRightJustify
            AutoSize = False
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clRed
            Font.Height = -15
            Font.Name = 'Tahoma'
            Font.Style = []
            ParentFont = False
          end
        end
        object grpEmissaoNfe: TGroupBox
          Left = 106
          Top = 24
          Width = 102
          Height = 39
          Caption = 'Emiss'#227'o'
          TabOrder = 1
          object lblEmissaoNfe: TLabel
            Left = 5
            Top = 16
            Width = 91
            Height = 18
            Alignment = taRightJustify
            AutoSize = False
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clRed
            Font.Height = -15
            Font.Name = 'Tahoma'
            Font.Style = []
            ParentFont = False
          end
        end
        object grpChaveNfe: TGroupBox
          Left = 209
          Top = 24
          Width = 396
          Height = 39
          Caption = 'Chave'
          TabOrder = 2
          object lblChaveNfe: TLabel
            Left = 5
            Top = 18
            Width = 388
            Height = 18
            Alignment = taRightJustify
            AutoSize = False
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clRed
            Font.Height = -15
            Font.Name = 'Tahoma'
            Font.Style = []
            ParentFont = False
          end
        end
      end
      object grpCorrecao: TGroupBox
        Left = 3
        Top = 87
        Width = 608
        Height = 105
        Caption = 
          'Corre'#231#227'o a ser considerada, texto livre. A corre'#231#227'o mais recente' +
          ' substitui as anteriores.'
        TabOrder = 1
        object memTextoCorrecao: TMemo
          Left = 2
          Top = 15
          Width = 604
          Height = 88
          Align = alClient
          MaxLength = 200
          TabOrder = 0
          OnEnter = memTextoCorrecaoEnter
          OnExit = memTextoCorrecaoEnter
          OnKeyPress = memTextoCorrecaoKeyPress
        end
      end
      object GroupBox1: TGroupBox
        Left = 1
        Top = 191
        Width = 608
        Height = 167
        Caption = 'Informa'#231#245'es para uso da CCe'
        Enabled = False
        TabOrder = 2
        object mtmInfoCCe: TMemo
          Left = 2
          Top = 15
          Width = 604
          Height = 150
          Align = alClient
          Font.Charset = ANSI_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Courier New'
          Font.Style = []
          Lines.Strings = (
            
              '          A Carta de Correcao e disciplinada pelo paragrafo 1o-A' +
              ' do art. 7o do '
            
              'Convenio S/N, de 15 de dezembro de 1970 e pode ser utilizada par' +
              'a regularizacao '
            
              'de erro ocorrido na emissao de documento fiscal, desde que o err' +
              'o nao esteja '
            'relacionado com:'
            
              '             I - as variaveis que determinam o valor do imposto ' +
              'tais como: base de '
            
              '                  calculo, aliquota, diferenca de preco, quantid' +
              'ade, valor da '
            '                  operacao ou da prestacao;'
            
              '            II - a correcao de dados cadastrais que implique mud' +
              'anca do remetente ou '
            '                  do destinatario;'
            '           III - a data de emissao oude saida.')
          ParentFont = False
          TabOrder = 0
        end
      end
    end
  end
  object grpMensagem: TGroupBox
    Left = 8
    Top = 454
    Width = 749
    Height = 32
    Caption = 'Mensagem'
    TabOrder = 3
    object lblMsg: TLabel
      Left = 79
      Top = 13
      Width = 567
      Height = 16
      ParentCustomHint = False
      AutoSize = False
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -12
      Font.Name = 'Tahoma'
      Font.Style = [fsBold, fsItalic]
      ParentFont = False
    end
  end
end
